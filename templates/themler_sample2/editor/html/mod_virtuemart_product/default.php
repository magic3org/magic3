...
<div data-slider-id="product_slider" class="data-control-id-2825 bd-productsslider-1" data-elements-per-row="<?php echo $products_per_row; ?>">
    <div class="bd-container-inner">
        <div class="data-control-id-200751 bd-block-5 <?php echo $params->get('moduleclass_sfx'); ?>">
            <?php if ($hasCaption) : ?>
            ...
            <div class="data-control-id-663667 bd-grid-26">
...
<form method="post" class="product" action="<?php echo JRoute::_ ('index.php'); ?>">
    ...
    <?php if (!VmConfig::get('use_as_catalog', 0)) : ?>
        ...
        <?php if (($stockhandle == 'disableit' or $stockhandle == 'disableadd') and ($productItems->productBuy->product_in_stock - $productItems->productBuy->product_ordered) < 1) : ?>
            <?php
                echo JHTML::link (JRoute::_ ('index.php?option=com_virtuemart&view=productdetails&layout=notify&virtuemart_product_id=' . $productItems->productBuy->virtuemart_product_id), vmText::_ ('COM_VIRTUEMART_CART_NOTIFY'), array('class' => 'data-control-id-280 bd-productbuy-1 bd-button notify'));
            ?>
        <?php else : ?>
            <?php
                ...
                if (!(VmConfig::get('askprice', true) and empty($tmpPrice))) {
                    if (isset($productItems->productBuy->orderable) && $productItems->productBuy->orderable) {
                        ...
                        $button = JHTML::link ($productItems->productBuy->link, JText::_ ('COM_VIRTUEMART_CART_ADD_TO'),
                            array('title' => $productItems->productBuy->product_name, 'class' => 'data-control-id-280 bd-productbuy-1 bd-button'));
