...
abstract class ArtxContentItem extends ArtxContentArticleBase
{
    ...
    {
        $images =
            isset($this->_article->images) && is_string($this->_article->images)
                ? json_decode($this->_article->images)
                : null;
        return [
    ...
    {
        $image = ['image' => '', 'float' => '', 'class' => '', 'caption' => '', 'alt' => ''];
        if (is_null($images)) {
            return $image;
        }
        $properties = [
    ...
    {
        $imgTagAttrs = ['src' => $image['image'], 'alt' => $image['alt']];
        if ($image['caption']) {
            $imgTagAttrs['class'] = 'caption';
            $imgTagAttrs['title'] = $image['caption'];
        }
        return artxTagBuilder(
            'div',
            ['class' => $image['class']],
            artxTagBuilder(
                'img',
                ['src' => $image['image'], 'alt' => $image['alt']] +
                    ($image['caption'] ? ['class' => 'caption', 'title' => $image['caption']] : [])
            )
        );
    }
}