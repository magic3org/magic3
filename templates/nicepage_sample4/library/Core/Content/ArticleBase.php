...
abstract class CoreContentArticleBase
{
    ...
    protected function __construct($component, $componentParams, $article, $articleParams)
    {
        ...
        $this->authorLink =
            strlen($this->author) && !empty($this->_article->contactid) && $this->_articleParams->get('link_author')
                ...
                : '';
        $this->shareLink =
            dirname(JURI::current()) .
            '/' .
            ContentHelperRoute::getArticleRoute($this->_article->slug, $this->_article->catid);
    }
    ...
    public function authorInfo($author, $authorLink)
    {
        if (strlen($authorLink)) {
            return JText::sprintf(
                ...
                JHtml::_('link', JRoute::_($authorLink), $author, ['itemprop' => 'url'])
            );
        }
        ...
        if (strlen($parentCategory)) {
            if (strlen($parentCategoryLink)) {
                echo '<a href="' .
                    $parentCategoryLink .
                    '" itemprop="genre">' .
                    $this->_component->escape($parentCategory) .
                    '</a>';
            } else {
        ...
        if (strlen($category)) {
            if (strlen($categoryLink)) {
                echo '<a href="' .
                    $categoryLink .
                    '" itemprop="genre">' .
                    $this->_component->escape($category) .
                    '</a>';
            } else {
    ...
    {
        return '<meta itemprop="interactionCount" content="UserPageVisits:' .
            $hits .
            '" />' .
