 ...
 * @version    SVN: $Id: mobilelink.js 3480 2010-08-16 09:27:01Z fishbone $
...
oRegex.UriProtocol = /^(((http|https|ftp|news):\/\/)|mailto:|tel:)/gi ;
...
oRegex.PhoneNumber = /^0\d{1,3}-?\d{2,4}-?\d{4}$/;
...
oParser.ParsePhoneNumber = function(phoneNumber)
{
	var oNumber = oRegex.PhoneNumber.exec(phoneNumber) ;
	if ( oNumber )
	{
		// Set the e-mail address.
		return oNumber[0];
...
function LoadSelection()
{
	...
	if ( sProtocol )
	{
		...
		else if (sProtocol == 'tel:')
		{
			sType = 'phone';
			GetE('txtPhoneNumber').value = oParser.ParsePhoneNumber(sUrl);
		}
...
function SetLinkType( linkType )
{
	...
	ShowE('divLinkTypePhone'	, (linkType == 'phone') ) ;
...
function Ok()
{
	...
	switch ( GetE('cmbLinkType').value )
	{
		...
		case 'anchor' :
			...
			break ;
		case 'phone':
			var sPhone = GetE('txtPhoneNumber').value ;
			if (sPhone.length == 0)
            {
                alert(FCKLang.DlnLnkMsgNoPhone);
                return false;
            }
            
            if (!oRegex.PhoneNumber.test(sPhone))
            {
                alert(FCKLang.DlnLnkMsgErrorPhone);
                return false;
            }
			sUri = 'tel:' + sPhone ;
			break ;
	...
	if ( !aHasSelection )
	{
		...
		switch ( GetE('cmbLinkType').value )
		{
			...
			case 'email':
				...
				break ;
            case 'phone':
                sInnerHtml = GetE('txtPhoneNumber').value;
				break;
		}
