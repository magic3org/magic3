 ...
 */
if (defined('M3_SYSTEM_DEFINE_INCLUDED')) {
    return;
}
...
if (version_compare(PHP_VERSION, '5.3.0') < 0) {
    die('PHP version error: ver=' . PHP_VERSION . '. Magic3 can move above version 5.3.0.');
}
// ######### システム起動状況 #########
...
define('M3_SYSTEM_VERSION', '3.13.0'); // システムのバージョン
define('M3_SYSTEM_RELEASE_DATE', '2023/2/15'); // システムのリリース日付
...
define(
    'M3_PATTERN_TAG_MACRO_USER_KEY',
    '/' . preg_quote(M3_TAG_START . M3_TAG_MACRO_USER_KEY) . '([A-Z0-9_]+)\|?(.*?)' . preg_quote(M3_TAG_END) . '/u'
); // ユーザ定義マクロパターン
...
// すべての機能タイプ
$M3_ALL_FEATURE_TYPE = [
    M3_VIEW_TYPE_DASHBOARD, // ダッシュボード
    M3_VIEW_TYPE_SEARCH, // 検索結果
    M3_VIEW_TYPE_COMMERCE, // Eコマース
    M3_VIEW_TYPE_CALENDAR,
]; // カレンダー
// すべてのコンテンツタイプ
$M3_ALL_CONTENT_TYPE = [
    M3_VIEW_TYPE_NEWS, // 新着情報
