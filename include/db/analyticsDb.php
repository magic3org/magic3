...
class analyticsDb extends BaseDb
{
    ...
    function getStatus($key)
    {
        ...
        $ret = $this->selectRecord($queryStr, [$key], $row);
        if ($ret) {
            $retValue = $row['as_value'];
        }
        return $retValue;
    }
    /**
    ...
    function updateStatus($key, $value)
    {
        ...
        if ($ret) {
            ...
            $queryStr .= 'WHERE as_id = ? ';
            $ret = $this->execStatement($queryStr, [$value, $now, $key]);
        } else {
            ...
            $queryStr .= '?, ?, ?';
            $queryStr .= ')';
            $ret = $this->execStatement($queryStr, [$key, $value, $now]);
        }
        // トランザクション確定
        if ($startTran) {
    ...
    function getOldAccessLog(&$row)
    {
		...
		*/
        ...
        $ret = $this->selectRecord($queryStr, [], $row);
        if ($ret) {
            $serial = $row['m'];
        }
        ...
        $ret = $this->selectRecord($queryStr, [$serial], $row);
        return $ret;
    }
    /**
     * 日付以前の古いアクセスログの日数を取得
    ...
    public function getOldAccessLogDayCount($date)
    {
        ...
        $endDt = $endDate . ' 0:0:0';
        $queryStr =
            'SELECT MIN(al_dt) AS first_dt FROM _access_log WHERE al_dt < ?';
        $params[] = $endDt;
    ...
    public function calcDatePv($date)
    {
        ...
        $ret = $this->execStatement($queryStr, [$date]);
        if (!$ret) {
            return false;
        }
        ...
        for ($i = 0; $i < 24; $i++) {
            ...
            $params = [];
            $queryStr =
                'SELECT COUNT(*) AS total,al_uri,al_path FROM _access_log ';
            ...
            if ($ret) {
                ...
                for ($j = 0; $j < count($rows); $j++) {
                    ...
                    // URLの長さのチェック
                    $url = $this->makeTruncStr($uri, self::MAX_URL_LENGTH);
                    if (empty($url)) {
                        $url = '/';
                    } // URLが空の場合は「/」とみなす
                    ...
                    if ($url != $uri) {
                        // URLが長いときは省略形で登録
                        $queryStr =
                            'SELECT ap_serial, ap_count FROM _analyze_page_view ';
                        $queryStr .= 'WHERE ap_type = ? ';
                        $queryStr .= 'AND ap_url = ? ';
                        $queryStr .= 'AND ap_date = ? ';
                        $queryStr .= 'AND ap_hour = ?';
                        $ret = $this->selectRecord(
                            $queryStr,
                            [0 /*すべてのデータ*/, $url, $date, $i],
                            $row
                        );
                        if ($ret) {
                            ...
                            $ret = $this->execStatement($queryStr, [
                                $count,
                                $serial,
                            ]);
                            if (!$ret) {
        ...
        $params = [];
        $queryStr =
            'SELECT COUNT(DISTINCT al_session) AS total,al_uri,al_path FROM _access_log ';
        $queryStr .= 'WHERE (? <= al_dt AND al_dt < ?) ';
        $params[] = $startDt;
        $params[] = $endDt;
        // ### コマンド実行のログは除く ###
        ...
        if ($ret) {
            ...
            for ($j = 0; $j < count($rows); $j++) {
                ...
                if ($url != $uri) {
                    // URLが長いときは省略形で登録
                    $queryStr =
                        'SELECT aa_serial, aa_count FROM _analyze_daily_count ';
        ...
        $queryStr =
            'SELECT COUNT(DISTINCT al_session) AS total, al_path FROM _access_log '; // 2011/6/2 PostgreSQL9対応
        $queryStr .= 'WHERE (? <= al_dt AND al_dt < ?) ';
        $params[] = $startDt;
        $params[] = $endDt;
        // ### コマンド実行のログは除く ###
        ...
        $queryStr =
            'SELECT COUNT(DISTINCT al_session) AS total FROM _access_log '; // OK
        $queryStr .= 'WHERE (? <= al_dt AND al_dt < ?) ';
        $params[] = $startDt;
        $params[] = $endDt;
        // ### コマンド実行のログは除く ###
        ...
        $params = [];
        $queryStr =
            'SELECT COUNT(DISTINCT al_cookie_value) AS total,al_uri,al_path FROM _access_log ';
        $queryStr .= 'WHERE (? <= al_dt AND al_dt < ?) ';
        $params[] = $startDt;
        $params[] = $endDt;
        // ### コマンド実行のログは除く ###
        ...
        $queryStr =
            'SELECT COUNT(DISTINCT al_cookie_value) AS total, al_path FROM _access_log '; // 2011/6/2 PostgreSQL9対応
        $queryStr .= 'WHERE (? <= al_dt AND al_dt < ?) ';
        $params[] = $startDt;
        $params[] = $endDt;
        // ### コマンド実行のログは除く ###
        ...
        $queryStr =
            'SELECT COUNT(DISTINCT al_cookie_value) AS total FROM _access_log '; // OK
        $queryStr .= 'WHERE (? <= al_dt AND al_dt < ?) ';
        $params[] = $startDt;
        $params[] = $endDt;
        // ### コマンド実行のログは除く ###
    ...
    function isExistsCookieValueInAccessLog($cookieValue, $serialNo = -1)
    {
        $params = [];
        $queryStr = 'SELECT * FROM _access_log ';
        $queryStr .= 'WHERE al_cookie_value = ? ';
        $params[] = $cookieValue;
        if ($serialNo >= 0) {
            $queryStr .= 'AND al_serial < ? ';
        }
        $params[] = $serialNo; // シリアルNoで制限
    ...
    function updateAccessLog($serialNo, $isFirst, $isCrawler)
    {
        ...
        $queryStr .= 'al_crawler = ? ';
        $queryStr .= 'WHERE al_serial = ?';
        $params = [intval($isFirst), intval($isCrawler), $serialNo];
