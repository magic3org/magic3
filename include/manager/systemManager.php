...
{
    private $_systemConfigArray = []; // システム定義値
    private $_siteDefArray = []; // サイト定義値
    ...
    function __construct()
    {
        ...
        $this->acceptLanguage = []; // アクセス可能言語
        $this->systemLanguages = ['ja', 'en']; // システムで利用可能な言語
    ...
    {
        if (isset($this->_systemConfigArray[$key])) {
            return $this->_systemConfigArray[$key];
        } else {
            return '';
        }
    }
        ...
        if ($ret) {
            ...
            if (!defined('M3_STATE_IN_INSTALL')) {
                ...
                if ($serverUrl != M3_SYSTEM_ROOT_URL || $serverDir != M3_SYSTEM_ROOT_PATH) {
                    ...
                    if ($ret) {
                        $this->_systemConfigArray[self::CF_SERVER_URL] = M3_SYSTEM_ROOT_URL;
                    }
                    $ret = $this->systemDb->updateSystemConfig(self::CF_SERVER_DIR, M3_SYSTEM_ROOT_PATH);
                    if ($ret) {
                        $this->_systemConfigArray[self::CF_SERVER_DIR] = M3_SYSTEM_ROOT_PATH;
                    }

                    $errMsg =
                        'DBの自動修正: システム設定マスター(_system_config)のサーバURL(server_url)またはサーバディレクトリ(server_dir)が変更されているので自動修正しました。URL: ' .
                        $serverUrl .
                        '=>' .
                        M3_SYSTEM_ROOT_URL .
                        ', ディレクトリ: ' .
            ...
            if ($this->getSystemConfig(self::CF_PERMIT_INIT_SYSTEM) == '1') {
                // システム初期化可能かどうか
            ...
            if ($this->pageCacheLifetime < 0) {
                $this->pageCacheLifetime = self::DEFAULT_PAGE_CACHE_LIFETIME;
            }
            if ($this->getSystemConfig(self::CF_HIERARCHICAL_PAGE) == '1') {
                $this->hierarchicalPage = true;
            } // 階層化ページを使用するかどうか
    ...
    function defaultTheme()
    {
        ...
        if (!isset($theme)) {
            $theme = $this->getSystemConfig(self::CF_DEFAULT_THEME);
        }
        return $theme;
    }
    /**
    ...
    {
        if ($langId == $this->getSystemConfig(self::CF_DEFAULT_LANG)) {
            return false;
        }
    ...
    {
        $limit =
            convBytes(ini_get('upload_max_filesize')) > convBytes(ini_get('post_max_size'))
    ...
    function getSystemManagerEnableTask()
    {
        ...
        if (!isset($enableTaskArray)) {
            ...
            if (!empty($enableTask)) {
                $enableTaskArray = explode(',', $enableTask);
            }
        }
    ...
    function recoverJQueryVersion($version)
    {
        ...
        if ($ret) {
            ...
            $errMsg =
                'DBの自動修正: システム設定マスター(_system_config)のjQueryのバージョン(jquery_version)を自動修正しました。バージョン: ' .
                $version;
            $this->gLog->error(__METHOD__, $errMsg);
        }
        return $ret;
