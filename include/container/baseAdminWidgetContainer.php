    ...
    function __assign()
    {
        ...
        if ($this->gPage->getUseHelp()) {
            ...
            if (method_exists($this, '_setHelp')) {
                $helpId = $this->_setHelp(null, $param);
                $helpKeys = $this->gInstance
                    ->getHelpManager()
                    ->loadHelp($this->gEnv->getCurrentWidgetId(), $helpId, $this);
            }
    ...
    function getAdminUrlWithOptionParam($withPageDef = false)
    {
        ...
        if ($cmd == M3_REQUEST_CMD_CONFIG_WIDGET) {
            ...
            if ($withPageDef) {
                ...
                if (!empty($configId)) {
                    $url .= '&' . M3_REQUEST_PARAM_PAGE_DEF_CONFIG_ID . '=' . $configId;
                } // 定義ID
                $defSerial = $this->gRequest->trimValueOf(M3_REQUEST_PARAM_PAGE_DEF_SERIAL);
                if (!empty($defSerial)) {
                    $url .= '&' . M3_REQUEST_PARAM_PAGE_DEF_SERIAL . '=' . $defSerial;
                } // 画面定義シリアル番号
            }
        } else {
