...
class BaseFrameContainer extends _Core
{
    ...
    function process($request)
    {
        ...
        if (!defined('M3_STATE_IN_INSTALL')) {
            if (
                ($this->gEnv->canUseDb() && $this->gSystem->canInitSystem()) || // システム初期化モードのとき
                !$this->gConfig->isConfigured()
            ) {
                // 設定ファイルに設定がないとき(初回インストール)

                // 旧システムがある場合はadminディレクトリまでアクセスしてインストーラを実行。
                $isRedirect = false;
                if ($this->_isExistsOldSystemDir()) {
                    // ディレクトリアクセス権がない場合も旧システムが存在するものとしてリダイレクトを行う
                    if ($this->gEnv->isAdminDirAccess()) {
                        $isRedirect = true;
                    }
                } else {
                    $isRedirect = true;
                ...
                return;
            } elseif ($this->gConfig->isConfigured() && !$this->gEnv->canUseDb()) {
                // DB接続失敗のとき
                if ($this->gEnv->isAdminDirAccess()) {
                    // 管理画面の場合のみインストーラ起動
                    // インストーラファイルがない場合は回復
        ...
        if (!defined('M3_STATE_IN_INSTALL')) {
            ...
            if (!$this->gEnv->isAdminDirAccess()) {
                // 管理画面以外の場合
                if ($this->gEnv->getCanChangeLang() && $this->gEnv->isMultiLanguageSite()) {
                    // 言語変更可能で多言語対応サイトのとき
            ...
            if ($canAccess) {
                ...
                if (!$isPublicUrl && !$isSystemManageUser) {
                    // システム運用可能ユーザかどうか
                    $canAccess = false;
            ...
            if ($canAccess) {
                ...
                if (method_exists($this, '_checkAccess')) {
                    ...
                    if (
                        ...
                        ($cmd == M3_REQUEST_CMD_CONFIG_WIDGET || // ウィジェットの設定
                            $cmd == M3_REQUEST_CMD_SHOW_POSITION_WITH_WIDGET)
                    ) {
                        // 表示位置を表示するとき(ウィジェット付き)
                        // 管理画面で処理
                        $toAdminType = 1; // ログイン画面へ
                    }
                } else {
                    // _checkAccess()がないときは、標準のアクセス制御
                    // フロント画面へのアクセスを制御
                    $canAccess = $this->_accessSite($request); // サイト公開制御
                    if ($canAccess) {
                        if (
                            $cmd == M3_REQUEST_CMD_LOGIN || // ログイン画面を表示のとき
                            $cmd == M3_REQUEST_CMD_LOGOUT
                        ) {
                            // ログアウトのとき
                        ...
                        // サイトアクセスできない場合は、管理画面でメッセージを表示
                        if (
                            $cmd == M3_REQUEST_CMD_LOGIN || // ログイン画面を表示のとき
                            $cmd == M3_REQUEST_CMD_LOGOUT || // ログアウトのとき
                            $cmd == M3_REQUEST_CMD_PREVIEW
                        ) {
                            // サイトのプレビューを表示
                            $toAdminType = 1; // ログイン画面へ
                        } else {
                ...
                if ($this->gEnv->isServerConnector()) {
                    ...
                    if ($isSystemAdmin && $cmd != M3_REQUEST_CMD_LOGOUT) {
                        $canAccess = true;
                    } // 2011/8/31 システム管理者のみに変更
                }
            }
            // #################### アクセスログ記録 #######################
            // DBが使用可能であれば、ログイン処理終了後、アクセスログを残す
            if ($this->gEnv->canUseDb()) {
                $this->gAccess->accessLog();
            }
            ...
            if (!$canAccess) {
                ...
                if ($isErrorAccess) {
                    $this->gOpeLog->writeUserAccess(
                        __METHOD__,
                        '不正なアクセスを検出しました。' . $errMessage,
                        2201,
                        'アクセスをブロックしました。URL: ' .
                            $this->gEnv->getCurrentRequestUri() .
                            ', ' .
                            $messageDetail
                    );
                }
            ...
            // #################### URLの遷移 #######################
            if ($this->gSystem->smartphoneAutoRedirect()) {
                // スマートフォン自動遷移を行う場合
        ...
        if ($cmd == M3_REQUEST_CMD_INIT_DB) {
            // DB初期化オペレーションのとき
        } elseif ($cmd == M3_REQUEST_CMD_SHOW_POSITION) {
            // 表示位置を表示するとき
            // 管理者権限がある場合のみ実行可能
            //if ($isSystemAdmin){
            if ($isSystemManageUser) {
                // システム運用者で表示可能(2018/8/5変更)
        ...
        } elseif ($cmd == M3_REQUEST_CMD_SHOW_POSITION_WITH_WIDGET) {
            // 表示位置を表示するとき(ウィジェット付き)
            // 管理者権限がある場合のみ実行可能
            //			if ($isSystemAdmin){
            if ($isSystemManageUser) {
        ...
        } elseif ($cmd == M3_REQUEST_CMD_GET_WIDGET_INFO) {
            // ウィジェット各種情報取得(AJAX用)
            // ウィジェット情報取得
            $this->gPage->getWidgetInfoByAjax($request);
            return;
        } elseif ($cmd == M3_REQUEST_CMD_SHOW_PHPINFO) {
            // phpinfoの表示
            // phpinfo画面を表示
            $this->_showPhpinfoPage($request);
            return;
        } elseif ($cmd == M3_REQUEST_CMD_FIND_WIDGET) {
            // ウィジェットを検索し、前面表示
            // 目的のウィジェットのあるページサブIDへ遷移
            $this->gPage->redirectToUpdatedPageSubId($request);
            return;
        } elseif ($cmd == M3_REQUEST_CMD_SHOW_WIDGET) {
            // ウィジェットの単体表示
            $createPage = false; // ウィジェット単体処理モードに設定
            $this->gPage->showWidget(); // ウィジェット表示
        } elseif ($cmd == M3_REQUEST_CMD_CONFIG_WIDGET) {
            // ウィジェットの設定管理
            $createPage = false; // ウィジェット単体処理モードに設定
            $this->gPage->showWidget(); // ウィジェット表示
        } elseif ($cmd == M3_REQUEST_CMD_DO_WIDGET) {
            // ウィジェット単体オペレーション
            $createPage = false; // ウィジェット単体処理モードに設定

            // ウィンドウオープンタイプ指定のときは、テンプレートを表示する
            if (!empty($openBy)) {
                $this->gPage->showWidget();
            } // ウィジェット表示
        } elseif ($cmd == M3_REQUEST_CMD_RSS) {
            // RSS配信
            $createPage = false; // ウィジェット単体処理モードに設定
        } elseif ($cmd == M3_REQUEST_CMD_CSS) {
            // CSS生成
        } elseif ($cmd == M3_REQUEST_CMD_CONFIG_TEMPLATE) {
            // テンプレートの設定
            $createPage = false; // ウィジェット単体処理モードに設定
            $this->gPage->showWidget(); // ウィジェット表示
        } elseif ($this->gEnv->isServerConnector()) {
            // サーバ接続の場合
            $createPage = false; // ウィジェット単体処理モードに設定
        }
        ...
        if ($createPage) {
            ...
            if ($cmd != M3_REQUEST_CMD_CSS) {
                // 画面出力(CSS生成以外)のとき
                // オプション出力(時間計測等)追加
                echo $this->gPage->getOptionContents($request);
            }
        } else {
            // 画面を作成しない場合(ウィジェット単体実行,RSS配信,サーバ接続)のとき
            // ###################ウィジェット指定で出力の場合####################
            ...
            if (
                ...
                if (empty($widgetId)) {
                    ...
                    return;
                } elseif ($this->_db->getWidgetInfo($widgetId, $row)) {
                    // ウィジェット情報取得
                    if ($cmd == M3_REQUEST_CMD_DO_WIDGET && !$row['wd_enable_operation']) {
                        // ウィジェット単体実行
                        // アクセスエラーのログを残す
                        ...
                        $this->gOpeLog->writeUserAccess(
                            __METHOD__,
                            'このウィジェットはRSS配信できません。(ウィジェットID: ' . $widgetId . ')',
                            2200,
                    ...
                    $this->gOpeLog->writeUserAccess(
                        __METHOD__,
                        'このウィジェットは実行許可がありません。(ウィジェットID: ' . $widgetId . ')',
                        2200,
            ...
            if ($this->gEnv->isServerConnector()) {
                ...
                if (!$isSystemManageUser && !$this->_db->canAccessWidget($widgetId)) {
                    ...
                    $this->gOpeLog->writeUserAccess(
                        __METHOD__,
                        'ウィジェットへの不正なアクセスを検出しました。(ウィジェットID: ' . $widgetId . ')',
                        2200,
            ...
            // サブクラスの前処理を実行
            if (method_exists($this, '_preBuffer')) {
            ...
            if ($this->gEnv->isServerConnector()) {
                ...
                $widgetIndexFile =
                    $this->gEnv->getWidgetsPath() . '/' . self::ADMIN_WIDGET_ID . '/' . M3_FILENAME_INDEX;
                ...
                if (file_exists($widgetIndexFile)) {
                    ...
                    $serial = 0; // シリアル番号
                    if (
                        $this->_db->getPageDefOnPageByWidgetId(
                            $this->gEnv->getCurrentPageId(),
                            $this->gEnv->getCurrentPageSubId(),
                            $widgetId,
                            $row
                        )
                    ) {
                ...
                if ($cmd == M3_REQUEST_CMD_CONFIG_WIDGET) {
                    ...
                    // テンプレート設定画面を取得
                    $widgetIndexFile =
                        $this->gEnv->getTemplatesPath() . '/' . $templateId . '/admin/' . M3_FILENAME_INDEX;
                } else {
                    $widgetIndexFile = $this->gEnv->getWidgetsPath() . '/' . $widgetId . '/' . M3_FILENAME_INDEX;
                }
                if (file_exists($widgetIndexFile)) {
            ...
            // サブクラスの後処理の呼び出し
            if (method_exists($this, '_postBuffer')) {
            ...
            if (
                ...
                ($cmd == M3_REQUEST_CMD_DO_WIDGET && !empty($openBy))
            ) {
                // ウィンドウオープンタイプ指定でウィジェット単体実行のとき
    ...
    function _createPage($request, $curTemplate, $subTemplateId = '')
    {
        ...
        // バッファリングの準備
        if (method_exists($this, '_prepareBuffer')) {
            $this->_prepareBuffer($request);
        }
        ...
        if ($convType == 100) {
            ...
            // プラグイン初期化
            $GLOBALS['wp_plugin_paths'] = []; // $wp_plugin_pathsは未使用?
            ...
            switch ($contentType) {
                case M3_VIEW_TYPE_CONTENT: // 汎用コンテンツ
                    if ($firstKey == M3_REQUEST_PARAM_CONTENT_ID || $firstKey == M3_REQUEST_PARAM_CONTENT_ID_SHORT) {
                        // コンテンツIDのとき
                        // ページタイプを設定
                ...
                case M3_VIEW_TYPE_PRODUCT: // 製品
                    if ($firstKey == M3_REQUEST_PARAM_PRODUCT_ID || $firstKey == M3_REQUEST_PARAM_PRODUCT_ID_SHORT) {
                        // 製品IDのとき
                        // ページタイプを設定
                ...
                case M3_VIEW_TYPE_BLOG: // ブログ
                    if (
                        $firstKey == M3_REQUEST_PARAM_BLOG_ENTRY_ID ||
                        $firstKey == M3_REQUEST_PARAM_BLOG_ENTRY_ID_SHORT
                    ) {
                        // ブログ記事IDのとき
                        // ページタイプを設定
                        $GLOBALS['gContentApi']->setPageType('single');
                        ...
                        if (!empty($value)) {
                            ...
                            } // カテゴリー用のテンプレートが取得できない場合はアーカイブ用テンプレートを取得
                            if (!empty($template)) {
                        ...
                        if (!$pageTypeDefined) {
                            ...
                            if (!empty($year)) {
                                ...
                                if (empty($template)) {
                                    $template = get_archive_template();
                                } // 年月日用のテンプレートが取得できない場合はアーカイブ用テンプレートを取得
                                if (!empty($template)) {
                                    $wpIndexFile = $template;
                                }
                ...
                default:
                    ...
                    if (!empty($value)) {
                        ...
                        if (empty($subId)) {
                            $subId = $this->_db->getSubPageIdWithContent(
                                M3_VIEW_TYPE_SEARCH,
                                $this->gEnv->getCurrentPageId()
                            ); // ページサブIDを取得
                            if (!empty($subId)) {
                                ...
                                $redirectUrl .=
                                    '&' .
                                    M3_REQUEST_PARAM_OPERATION_TASK .
                                    '=search&' .
                                    M3_REQUEST_PARAM_KEYWORD .
                                    '=' .
                                    urlencode($value); // 検索キーワード
                    ...
                    if (!empty($pageInfo)) {
                        ...
                        if (!empty($pageTitle)) {
                            $GLOBALS['gContentApi']->setPostTitle($pageTitle);
                        }
                    }
            ...
            // サブテンプレート用の設定
            if ($this->gEnv->getCurrentTemplateGenerator() == M3_TEMPLATE_GENERATOR_THEMLER) {
                // Themlerテンプレートの場合はサブテンプレート用のパラメータを設定
                // JRequest経由でレンダー側にサブテンプレートIDを渡す
                if (!empty($subTemplateId)) {
                ...
                // サブテンプレートIDを埋め込む
                if (!empty($subTemplateId)) {
                    $this->setBuffer('<!--TEMPLATE ' . $subTemplateId . ' /-->', 'component');
                }
            }
            ...
            // デフォルト(Joomla!v1.0テンプレート)テンプレートのとき(PC用および携帯用)
            // Joomla!テンプレート共通の設定
            define('_JEXEC', 1);
        ...
        // ################### テンプレート読み込み ###################
        // フルパスに変換
        if (strStartsWith($defaultIndexFile, '/')) {
            // フルパス起動(WordPressテンプレート)のとき
        ...
        if ($this->gPage->isRedirect()) {
            return '';
        } // リダイレクトの場合ob_end_clean()を実行すると、ログインできないことがあるのでここで終了(2011/11/11)
    ...
    function _defineTemplate($request, &$subTemplateId)
    {
        ...
        if (empty($curTemplate)) {
            if (
                ...
                // 取得できなければデフォルトを取得
                if (empty($curTemplate)) {
                    if ($pageId == $this->gEnv->getDefaultPageId()) {
                        // 通常サイトのとき
                        $curTemplate = $this->gSystem->defaultTemplateId();
                        $subTemplateId = $this->gSystem->defaultSubTemplateId();
                    } elseif ($pageId == $this->gEnv->getDefaultSmartphonePageId()) {
                        // スマートフォン用サイトのとき
                        $curTemplate = $this->gSystem->defaultSmartphoneTemplateId(); // スマートフォン用デフォルトテンプレート
                    } elseif (
                        $pageId == $this->gEnv->getDefaultAdminPageId() || // 管理サイトのとき
                        $pageId == $this->gEnv->getDefaultRegistPageId()
                    ) {
                        // 登録サイトのとき
                        $curTemplate = $this->gSystem->defaultAdminTemplateId();
                    } elseif (empty($pageId)) {
                        // ページIDが指定されていないときは、ウィジェットを表示しないテンプレートのみの表示
                        // URLの引数でテンプレートIDが指定されている場合は設定
                        //					$templateId = $request->trimValueOf(M3_REQUEST_PARAM_TEMPLATE_ID);		// テンプレートIDを取得
    ...
    function _accessSite($request)
    {
        ...
        if ($isOpen) {
            ...
            if ($this->gEnv->getIsPcSite()) {
                if ($this->gSystem->sitePcInPublic()) {
                    return true;
                }
            } elseif ($this->gEnv->getIsSmartphoneSite()) {
                if ($this->gSystem->siteSmartphoneInPublic()) {
                    return true;
                }
            }
            return false;
    ...
    function _showSystemPage($request, $type)
    {
        ...
        if ($type == 1) {
            ...
            if (empty($curTemplateId)) {
                ...
                if (!file_exists($templateIndexFile)) {
                    $curTemplateId = self::SYSTEM_TEMPLATE;
                } // システム画面用テンプレート
            }
        }
    ...
    function _isExistsOldSystemDir()
    {
        ...
        if (@is_dir($parentDir)) {
            ...
            // 親ディレクトリへのアクセス権がない場合は旧システムが存在すると判断する
            return true;
        }
    }
    /**
    ...
    function getBuffer($type = null, $name = null, $attribs = [])
    {
    ...
    public function addStyleSheet($url, $options = [], $attribs = [])
    {
        ...
        if (is_string($options)) {
            JLog::add(
                'The addStyleSheet method signature used has changed, use (url, options, attributes) instead.',
                JLog::WARNING,
                'deprecated'
            );
        ...
        $this->_styleSheets[$url] = isset($this->_styleSheets[$url])
            ? array_replace($this->_styleSheets[$url], $attribs)
            : $attribs;
