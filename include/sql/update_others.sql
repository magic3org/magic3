...
('helplink', 11,     11,        'task=configsys',       '共有SSLを設定するには？', '「システム基本設定」画面の「SSL」の「一般画面にSSLを使用」にチェックを入れ、「共有SSLのルートURL」に共有SSL用のURLを設定します。'),
('helplink', 12,     12,        'task=configsys',       'イントラネット運用するには？', '「システム基本設定」画面の「ネットワーク」の「イントラネット運用」にチェックを入れます。Googleマップ等外部サービスにアクセスする機能が停止になります。');
