...
('helplink', 3,     3,        'task=initwizard_content',       '表示するコンテンツや機能を選択するには？', '「システム初期化ウィザード」の「コンテンツ」画面を設定し、次へ進みます。'),
('helplink', 4,     4,        'task=initwizard_page2',       'デフォルトページを変更するには？', '「システム初期化ウィザード」の「ページ2」画面を設定し、次へ進みます。'),
('helplink', 5,     5,        'task=initwizard_menu',       'メニューを階層化して定義するには？', '「システム初期化ウィザード」の「メニュー」画面を設定し、次へ進みます。'),
...
('helplink', 13,     13,        'task=configsys',       '共有SSLを設定するには？', '「システム基本設定」画面の「SSL」の「一般画面にSSLを使用」にチェックを入れ、「共有SSLのルートURL」に共有SSL用のURLを設定します。'),
