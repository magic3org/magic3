...
// +----------------------------------------------------------------------+
//
// $Id: BBCodeParser2.php 6079 2013-06-05 00:45:38Z fishbone $
//
...
class HTML_BBCodeParser2
{
    ...
    {
		global $gEnvManager;	// added for Magic3 by naoki
		
        $filter = ucfirst($filter);
        if (!array_key_exists($filter, $this->_filters)) {
            $class = 'HTML_BBCodeParser2_Filter_'.$filter;
            //@include_once 'HTML/BBCodeParser2/Filter/'.$filter.'.php';
			include_once $gEnvManager->getLibPath() . '/HTML_BBCodeParser2-0.1.0/BBCodeParser2/Filter/'.$filter.'.php';	// modified for Magic3 by naoki
