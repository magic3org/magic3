...
class LoggerPropertyConfigurator extends LoggerConfigurator {
     ...
     * @static
     */
    //static function configure($url = '')		// C³(2006/12/11)
	...
	*/
        $configurator = new LoggerPropertyConfigurator();// C³(2006/12/11)
        $repository = LoggerManager::getLoggerRepository();
        return $configurator->doConfigure($url, $repository);
    }
	...
	function parseCategory($props, &$logger, $optionKey, $loggerName, $value)		// C³(2006/12/12)
    {
        LoggerLog::debug(
        ...
        while($appenderName = next($st)) {
			...
			$appender = $this->parseAppender($props, $appenderName);		// C³(2006/12/12)
            if($appender !== null) {
    ...
    function parseAppender($props, $appenderName)		// C³(2006/12/12)
    {
        //$appender =& LoggerAppender::singleton($appenderName);
        $appender = LoggerAppender::singleton($appenderName);	// C³(2006/12/12)
        if($appender !== null) {
            LoggerLog::debug(
        ...
        if (!empty($appenderClass)) {
			...
			$appender = LoggerAppender::singleton($appenderName, $appenderClass);		// C³(2006/12/12)
            if($appender === null) {
                LoggerLog::warn(
                    "LoggerPropertyConfigurator::parseAppender() ".
