...
class LoggerOptionConverter {
    ...
    */
     ...
     * @static
     */
    //function toLevel($value, $defaultValue)		// èCê≥(2006/12/12)
	static function toLevel($value, $defaultValue)
    {
        ...
        if (class_exists($clazz)) {
			...
			if (!($result instanceof LoggerLevel)){		// èCê≥(2006/12/12)
	...
	static function findAndSubst($key, $props)		// èCê≥(2006/12/12)
    {
        $value = @$props[$key];
        if(empty($value)) {
            return null;
	...
	static function substVars($val, $props = null)		// èCê≥(2006/12/12)
    {
