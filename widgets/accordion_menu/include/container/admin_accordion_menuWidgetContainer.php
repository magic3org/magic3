 ...
 * @version    SVN: $Id: admin_accordion_menuWidgetContainer.php 4943 2012-06-07 23:21:43Z fishbone $
 * @link       http://www.magic3.org
 */
require_once($gEnvManager->getContainerPath() . '/baseAdminWidgetContainer.php');
require_once($gEnvManager->getCurrentWidgetDbPath() . '/accordion_menuDb.php');

class admin_accordion_menuWidgetContainer extends BaseAdminWidgetContainer
{
	...
	const DEFAULT_CSS = "\$1 {\n    border:1px solid #008000;\n    margin:0px;\n    padding:0px;\n    text-indent:0px;\n    background-color:#8AC452;\n    width:150px;\n}\n\$1 a.head {\n    cursor:pointer;\n    border:1px solid #FFFFFF;\n    background:#008000 url([#M3_WIDGET_URL#]/images/collapsed.gif) no-repeat scroll 3px 4px;\n    color:#FFFFFF;\n    display:block;\n    font-weight:bold;\n    margin:0px;\n    padding:0px;\n    text-indent:14px;\n    text-decoration: none;\n}\n\$1 a.head:hover {\n    color:#FFF7C6;\n}\n\$1 a.selected {\n    background-image: url([#M3_WIDGET_URL#]/images/expanded.gif);\n}\n\$1 a.current {\n    background-color:#FFF7C6;\n}\n\$1 ul {\n    border-width:0px;\n    margin:0px;\n    padding:0px;\n    text-indent:0px;\n}\n\$1 li {\n    list-style:none outside none;\n    display:inline;\n}\n\$1 li li a {\n    color:#000000;\n    display:block;\n    text-indent:10px;\n    text-decoration: none;\n}\n\$1 li li a:hover {\n    background-color:#FFF7C6;\n    color:#FF0000;\n}\n";
	...
	function createDetail($request)
	{
		...
		if (empty($this->configId)){		// 新規登録の場合
			...
			$this->css = str_replace('$1', '#' . $this->cssId, self::DEFAULT_CSS);		// メニュー用CSS
			$this->serialNo = 0;
		} else {
			if ($replaceNew){
				$ret = $this->getPageDefParam($defSerial, $defConfigId, $this->paramObj, $this->configId, $targetObj);
		...
		if ($this->db->getChildMenuItems($menuId, $parantId, $rows)){
			...
			for ($i = 0; $i < $itemCount; $i++){
				...
				if ($row['md_type'] == 0){	// リンク項目のとき
					...
					//$treeHtml .= '<li><span>' . $this->convertToDispString($row['md_name']) . '</span></li>' . M3_NL;
