...
class admin_opelogWidgetContainer extends BaseAdminWidgetContainer
{
    ...
    private $logStatus; // 現在のログ表示ステータス(0=すべて、1=未確認のみ、2=確認済みのみ)
    private $serialArray;
    ...
    function createList($request)
    {
        ...
        if (empty($paramObj)) {
            ...
            if (!isset($listCount)) {
                $listCount = self::DEFAULT_LIST_COUNT;
            } // 取得数
            $viewCount = $paramObj->viewCount; // 表示数
            if (!isset($viewCount)) {
                $viewCount = self::DEFAULT_VIEW_COUNT;
            } // 表示数
        }
        $pageNo = 1;

        // 総数を取得
        $totalCount = $this->db->getOpeLogCount(
            $viewLevel,
            $this->logStatus,
            $this->ownerUserId
        );
        ...
        $endNo =
            $pageNo * $listCount > $totalCount
                ? $totalCount
                : $pageNo * $listCount; // 最後の行番号
        ...
        if (count($this->serialArray) == 0) {
            $this->tmpl->setAttribute('loglist', 'visibility', 'hidden');
        } // ログがないときは非表示
    ...
    function logListLoop($index, $fetchedRow, $param)
    {
        ...
        $iconTag =
            ...
            '" />';
        $accessLog = '';
        if (!empty($fetchedRow['ol_access_log_serial'])) {
            $accessLog = $this->convertToDispString(
                $fetchedRow['ol_access_log_serial']
            );
        }
        ...
        if (!empty($fetchedRow['ol_link'])) {
            $iconTag = $this->gDesign->createAdminPageLink(
                $iconTag,
                $fetchedRow['ol_link']
            );
        }
        ...
        if (intval(date('Y', strtotime($outputDate))) == $this->currentYear) {
            $dispDate = $this->convertToDispDateTime(
                $outputDate,
                11 /*年省略,0なし年月*/,
                10 /*時分表示*/
            );
        ...
        $row = [
            ...
            'message' => $this->convertToDispString(
                makeTruncStr($fetchedRow['ol_message'], self::MAX_MSG_LENGTH)
            ), // メッセージ
