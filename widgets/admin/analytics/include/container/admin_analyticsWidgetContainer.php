...
class admin_analyticsWidgetContainer extends BaseAdminWidgetContainer
{
    ...
    function __construct()
    {
        ...
        // グラフタイプ
        $this->graphTypeArray = [
            ['name' => 'ページビュー', 'value' => 'pageview'],
            ['name' => '訪問数', 'value' => 'visit'],
            ['name' => '訪問者数', 'value' => 'visitor'],
        ];

        // 期間タイプ
        $this->termTypeArray = [
            ['name' => '10日', 'value' => '10day'],
            ['name' => '30日', 'value' => '30day'],
            ['name' => '3ヶ月', 'value' => '3month'],
            ['name' => '6ヶ月', 'value' => '6month'],
            ['name' => '1年', 'value' => '1year'],
            ['name' => 'すべて', 'value' => self::TERM_TYPE_ALL],
        ];
        // Y軸の最大値
        //$this->yTickValueArray = array(1000000, 750000, 500000, 250000, 100000, 75000, 50000, 25000, 10000, 7500, 5000, 2500, 1000, 500, 100, 0);
        $this->yTickValueArray = [
            1000000,
            900000,
            750000,
            500000,
    ...
    function _assign($request, &$param)
    {
        ...
        if ($act == 'analytics_update' && $this->enableCalc) {
            // 集計処理可能で設定更新(再計算)のとき
            $messageArray = [];
            $ret = $this->gInstance
                ->getAnalyzeManager()
                ->updateAnalyticsData($messageArray);
            if ($ret) {
        ...
        if (empty($endDate)) {
            ...
            $yMax = 100;
            $lineDataScript .=
                M3_INDENT_SPACE . 'var ' . self::LINE_DATA_HEAD . ' = [[]];'; // ラインデータ
            ...
            if (!is_null($startDate)) {
                // 先頭の日付を修正
                if (strtotime($startDate) < strtotime($logStartDate)) {
            ...
            for ($i = 0; $i < count($this->pathArray); $i++) {
                ...
                switch ($this->graphType) {
                    case 'pageview': //ページビュー
                        // データ取得
                        $this->db->getPageViewByDate(
                            $pathParam,
                            $startDate,
                            $endDate,
                            [$this, 'pageViewDataLoop']
                        );
                        break;
                    //					case 'visit':			// 訪問数
                ...
                $nextStartDate = date(
                    'Y-m-d',
                    strtotime("$this->completedDate 1 day")
                ); // 処理終了日翌日
        ...
        $this->tmpl->addVar(
            '_widget',
            'date_title',
            $this->convertToDispString($dateTitle)
        ); // 集計日タイトル
