 ...
 * @version    SVN: $Id: user_contentContentWidgetContainer.php 3042 2010-04-17 12:24:42Z fishbone $
 * @link       http://www.m-media.co.jp
 */
require_once($gEnvManager->getCurrentWidgetContainerPath() .	'/user_contentBaseWidgetContainer.php');

class user_contentContentWidgetContainer extends user_contentBaseWidgetContainer
{
	private $langId;		// 言語ID
	...
	private $showMessage;		// メッセージを表示するかどうか
	...
	function _setTemplate($request, &$param)
	{
		...
		if (!$ret){
			$this->showMessage = true;		// メッセージを表示
			$this->SetMsg(self::MSG_APP_ERR, "該当するルームがありません");
			return 'message.tmpl.html';
		}
		
		if ($task == 'content_detail'){		// 詳細画面
			return 'content_detail.tmpl.html';
		} else {			// 一覧画面
			return 'content.tmpl.html';
		}
	}
	/**
	 * テンプレートにデータ埋め込む
	...
	function createList($request)
	{
		...
		// ルームIDを設定
		$this->tmpl->addVar('_widget', 'room_id', $this->roomId);	// ルームID表示
	...
	function roomListLoop($index, $fetchedRow, $param)
	{
		...
		$row = array(
			...
			'name' => $this->convertToDispString($id),		// 名前
