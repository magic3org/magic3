 ...
 * @version    SVN: $Id: staterateWidgetContainer.php 5436 2012-12-07 09:55:12Z fishbone $
 * @link       http://www.magic3.org
 */
require_once($gEnvManager->getContainerPath() . '/baseIWidgetContainer.php');

class staterateWidgetContainer extends BaseIWidgetContainer
{
	/**
	 * コンストラクタ
	 */
		...
		if ($act == 'calc'){		// 計算のとき
			...
			if ($useMin && $minPrice > 0){		// 購入最低額以上は無料のとき
				if ($productTotal >= $minPrice){
					...
					$price = $priceArray[$stateId];		// 都道府県の価格表から取得
				}
			} else {
				$price = $priceArray[$stateId];		// 都道府県の価格表から取得
			}
