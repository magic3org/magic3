			...
			if ($this->getMsgCount() == 0){
				...
				for ($i = 0; $i < count($listedItem); $i++){
					...
					case M3_VIEW_TYPE_MEMBER:			// 会員情報
						$this->db->updateWidgetCategoryVisible($contentType, false);
						break;
					case M3_VIEW_TYPE_PRODUCT:		// 製品
					case M3_VIEW_TYPE_COMMERCE:			// Eコマース
						$this->db->updateWidgetCategoryVisible(M3_VIEW_TYPE_COMMERCE, false);
						break;
					}
					...
					if ($itemValue){
						...
						if (!in_array($contentType, $oldContentType)){
							...
							for ($j = 0; $j < count($this->pageIdArray); $j++){
								...
								$ret = $this->db->getViewWidgetListByDeviceType($contentType, $j/*デバイスタイプ*/, $rows);
								if ($ret){
						...
						case M3_VIEW_TYPE_PHOTO:			// フォトギャラリー
						case M3_VIEW_TYPE_MEMBER:			// 会員情報
							$this->db->updateWidgetCategoryVisible($contentType, true);
							break;
						case M3_VIEW_TYPE_PRODUCT:		// 製品
						case M3_VIEW_TYPE_COMMERCE:			// Eコマース
							$this->db->updateWidgetCategoryVisible(M3_VIEW_TYPE_COMMERCE, true);
							break;
						}
					} else {
						// 変更状況に関わらず処理を行う
...
//						if (in_array($contentType, $oldContentType)){
						for ($j = 0; $j < count($widgetInfoRows); $j++){
							...
							if ($contentType == $widgetInfoRows[$j]['wd_content_type']){
								// ウィジェットをページから削除
								$ret = $this->db->delPageDefByWidgetId($widgetId);
							}
						}
...
//						}
	...
	function getSelectedContentType(&$widgetInfoRows)
	{
		...
		$ret = $this->db->getContentWidgetOnPage($this->langId, $this->pageIdArray, $contentType, $rows);
		if ($ret){
			$widgetInfoRows = $rows;
	...
	{
		// 使用ページにする
		$ret = $this->db->getPageIdRecord(1/*ページサブIDを指定*/, $pageSubId, $row);
		if ($ret){
			$ret = $this->db->updatePageId(1/*ページサブIDを指定*/, $pageSubId, $row['pg_name'], $row['pg_description'], $row['pg_priority'], $active, $row['pg_visible']);
		}
		return $ret;
	}
	/**
