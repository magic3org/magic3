...
class admin_mainPagedefWidgetContainer extends BaseAdminWidgetContainer
{
    ...
    function createView($request)
    {
        ...
        if (empty($this->pageSubId)) {
            $this->pageSubId = $this->gEnv->getDefaultPageSubIdByPageId($this->pageId);
        } // デフォルト値取得
        ...
        if ($act == 'changetemplate') {
            ...
            if (!empty($templateId)) {
                ...
                $ret = $this->db->getPageInfo($this->pageId, $this->pageSubId, $row);
                if ($ret) {
                    if (is_null($row['pn_template_id'])) {
                        // ページ情報レコードがない場合
                        $ret = $this->db->updatePageInfo(
                            $this->pageId,
                            $this->pageSubId,
                            '' /*コンテンツタイプ*/,
                            $templateId,
        ...
        } elseif ($act == 'importcontent') {
            // テンプレートのページコンテンツをインポート(Nicepageテンプレートのみ)
            // パラメータエラーチェック
            // 個別のテンプレート取得
            $pageTemplateId = '';
            $ret = $this->db->getPageInfo($this->pageId, $this->pageSubId, $row);
            if ($ret) {
                $pageTemplateId = $row['pn_template_id'];
            }

            if (!empty($pageTemplateId)) {
                // ページに個別のテンプレートが設定されている必要がある
                // テンプレート情報取得
                $ret = $this->db->getTemplate($pageTemplateId, $row);
                if ($ret && $row['tm_generator'] == M3_TEMPLATE_GENERATOR_NICEPAGE) {
                    $ret = $this->gInstance
                        ->getContentManager()
                        ->importPageContentFromTemplate($this->pageId, $this->pageSubId, $pageTemplateId);
                    if ($ret) {
                        // データインポート成功のとき
        ...
        // ページメインIDメニュー作成
        $this->db->getPageIdList([$this, 'pageIdLoop'], 0 /*ページID*/, $deviceType);

        // ページサブIDメニュー作成(ページメインIDを先に作成してから)。デバイスで共通。
        $this->db->getPageIdList(
            [$this, 'pageSubIdLoop'],
            1 /*サブページID*/,
            -1 /*デバイス関係なし*/,
            true /*メニューから選択可項目のみ*/
        ...
        // テンプレート選択メニュー作成
        $this->db->getAllTemplateList($deviceType, [$this, 'templateIdLoop']);
        ...
        if (!empty($this->subTemplateId)) {
            $defaultTemplate .= '(' . $this->subTemplateId . ')';
        } // サブテンプレートがある場合は付加
        ...
        $adminUrl =
            ...
            '?' .
            M3_REQUEST_PARAM_DEF_PAGE_ID .
            '=' .
            $this->pageId .
            '&' .
            M3_REQUEST_PARAM_DEF_PAGE_SUB_ID .
            '=' .
            $this->pageSubId;
    ...
    function createDetail($request)
    {
        ...
        $this->pageSubId = $request->trimValueOf('pagesubid'); // ページサブID
        if (empty($this->pageId)) {
            $this->pageId = $this->gEnv->getDefaultPageId();
        } // デフォルト値取得
        if (empty($this->pageSubId)) {
            $this->pageSubId = $this->gEnv->getDefaultPageSubIdByPageId($this->pageId);
        } // デフォルト値取得
        ...
        if ($act == 'updateline') {
            ...
            if ($this->db->getPageDef($this->serialNo, $row)) {
                ...
                if ($request->trimValueOf('item' . $selectedItemNo . '_shared') != 'on') {
                    $updatePageSubId = $this->pageSubId;
                }
        ...
        // 定義詳細一覧表示
        $this->db->getPageDefList([$this, 'pageListLoop'], $this->pageId, $this->pageSubId, $this->position);
        if (!$this->isExistsDefItems) {
            $this->tmpl->setAttribute('page_def_list', 'visibility', 'hidden');
        }

        // ポジションメニュー作成
        $this->db->getPagePositionList([$this, 'pagePositionLoop']);
    ...
    function pageListLoop($index, $fetchedRow, $param)
    {
        ...
        if (!$fetchedRow['wd_has_admin']) {
            $configButtonEnabled = 'disabled';
        }

        // 項目を画面に表示するかどうか
        $itemVisible = '';
        if ($fetchedRow['pd_visible']) {
            $itemVisible = 'checked';
        }
    ...
    function pageIdLoop($index, $fetchedRow, $param)
    {
        ...
        $name =
            $this->convertToDispString($fetchedRow['pg_id']) .
            ' - ' .
            $this->convertToDispString($fetchedRow['pg_name']); // ページ名
        $row = [
    ...
    function pageSubIdLoop($index, $fetchedRow, $param)
    {
        ...
        $pageTitle = '';
        $preTitle = '';
        if (!$fetchedRow['pg_visible']) {
            $preTitle .= self::TITLE_PRE_ICON_CLOSED;
        } // 「非公開」状態
        if ($value == $this->defaultPageSubId) {
            $preTitle .= self::TITLE_PRE_ICON_HOME;
        } // デフォルトページ(homeアイコン)
        if ($useSsl) {
            $preTitle .= self::TITLE_PRE_ICON_LOCK;
        } // SSL使用ページ(鍵アイコン)
        if (!$fetchedRow['pg_active']) {
            $preTitle .= self::TITLE_PRE_ICON_MINUS;
        } // 非表示ページ(非表示アイコン)
        ...
        // 表示ラベル
        if ($value == $this->defaultPageSubId) {
            $nameWithAttr .= ' [' . $this->_('Default') . ']';
        } // デフォルトのページサブIDのときは、説明を付加
        if (!$fetchedRow['pg_active']) {
            $nameWithAttr .= ' [' . $this->_('Unpublished') . ']';
        } // 非公開

        // テンプレートIDの表示
        $dispTemplateId = $templateId;
        if (!empty($subTemplateId)) {
            $dispTemplateId .= '(' . $subTemplateId . ')';
        }
    ...
    function templateIdLoop($index, $fetchedRow, $param)
    {
        ...
        if ($value == $this->templateId) {
            ...
            if (empty($this->templateTitle)) {
                ...
                } elseif ($generator == M3_TEMPLATE_GENERATOR_THEMLER) {
                    // Themler製テンプレートの場合
                    $this->templateTitle .= self::TEMPLATE_TYPE_LABEL_THEMLER;
                } elseif ($generator == M3_TEMPLATE_GENERATOR_NICEPAGE) {
                    // Nicepage製テンプレートの場合
        ...
        if ($type == 100) {
            ...
            foreach (['png', 'gif', 'jpg', 'jpeg'] as $ext) {
                $imgPath =
                    $this->gEnv->getTemplatesPath() .
                    '/' .
                    $value .
                    '/' .
                    self::TEMPLATE_THUMBNAIL_FILENAME_WP .
                    '.' .
                    $ext;
                if (file_exists($imgPath)) {
                    $imageUrl =
                        $this->gEnv->getTemplatesUrl() .
                        '/' .
                        $value .
                        '/' .
                        self::TEMPLATE_THUMBNAIL_FILENAME_WP .
        ...
        if ($value == $this->pageTemplateId) {
            ...
            $this->templateTitle =
                self::BUTTON_ICON_TEMPLATE_CHECK_WITH_HELP . $this->convertToDispString($this->pageTemplateId); // 選択テンプレートのタイトル
        ...
        $selectTag =
            '<span ' .
            $selectButtonHelp .
            '><a class="btn btn-sm btn-default" onclick="changePageTemplate(\'' .
            $value .
            '\');return false;">' .
            $selectButtonIcon .
            '</a></span>';
    ...
    function createSubTemplateMenu()
    {
        ...
        if (!empty($this->pageTemplateId)) {
            $selectedTemplateId = $this->pageTemplateId;
        } // 個別ページのテンプレートが選択されている場合は優先
        ...
        switch ($generator) {
            case M3_TEMPLATE_GENERATOR_THEMLER: // Themler製テンプレート
                ...
                $subTemplateInfoFile =
                    $this->gEnv->getTemplatesPath() . '/' . $selectedTemplateId . '/templates/list.php';
                if (is_readable($subTemplateInfoFile)) {
                    // サブテンプレート情報ファイル読み込み
                    require_once $subTemplateInfoFile;
                    if (!isset($this->subTemplateInfo)) {
                        $this->subTemplateInfo = $templatesInfo;
                    } // ローカル変数へコピー
				...
				$this->tmpl->parseTemplate('subtemplate_list', 'a');*/
                    ...
                    foreach ($this->subTemplateInfo as $key => $templateInfo) {
                        ...
                        if ($type == 'error404') {
                            continue;
                        } // エラーメッセージ表示用の404タイプのサブテンプレートは表示しない

                        $selected = '';
                        if (empty($this->pageTemplateId)) {
                            // ページ個別のテンプレートが設定されていない場合
                            if ($subTemplateId == $this->subTemplateId) {
                                $selected = 'selected';
                            } // サブテンプレートID
                        } else {
                            if ($subTemplateId == $this->pageSubTemplateId) {
                                $selected = 'selected';
                            } // サブテンプレートID
                        }
                        ...
                        $row = [
                            ...
                            'name' => $this->convertToDispString(
                                $templateInfo['defaultTemplateCaption'] . '(' . $templateInfo['fileName'] . ')'
                            ),
                            'selected' => $selected, // 選択中かどうか
                        ];
                        $this->tmpl->addVars('subtemplate_list', $row);
