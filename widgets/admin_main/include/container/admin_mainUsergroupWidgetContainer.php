...
class admin_mainUsergroupWidgetContainer extends admin_mainUserBaseWidgetContainer
{
	...
	function createList($request)
	{
		...
		// #### ユーザグループリストを作成 ####
		$this->_mainDb->getAllUserGroup($this->langId, array($this, 'groupListLoop'));// デフォルト言語で取得
	...
	function createDetail($request)
	{
		...
		if ($act == 'add'){		// 項目追加の場合
			...
			// 同じIDがある場合はエラー
			if ($this->_mainDb->getUserGroupById($id, $row)) $this->setMsg(self::MSG_USER_ERR, 'IDが重複しています');
			
			// エラーなしの場合は、データを登録
			if ($this->getMsgCount() == 0){
				$ret = $this->_mainDb->addUserGroup($id, $this->langId, $name, $index, $newSerial);
				if ($ret){
			...
			$ret = $this->_mainDb->delUserGroupBySerial(array($this->serialNo));
			if ($ret){		// データ削除成功のとき
				$this->setGuidanceMsg('データを削除しました');
			} else {
				$this->setAppErrorMsg('データ削除に失敗しました');
			...
			if (empty($this->serialNo)){		// シリアル番号
				$id		= '';		// 識別ID
				$name = '';		// 名前
				$index = $this->_mainDb->getUserGroupMaxIndex($this->langId) + 1;	// 表示順
			} else {
				$replaceNew = true;			// データを再取得
			}
		}
