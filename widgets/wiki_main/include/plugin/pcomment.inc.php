...
/*	if (! isset($vars['msg']) || $vars['msg'] == '') return array();
	...
	*/
	if (WikiParam::getMsg() == '') return array();
	...
	header('Location: ' . get_script_uri() . WikiParam::convQuery('?' . rawurlencode($refer), false));
	//exit;
...
/*	$refer = isset($vars['refer']) ? $vars['refer'] : '';
	...
	$msg = str_replace('$msg', rtrim(WikiParam::getMsg()), PLUGIN_PCOMMENT_FORMAT_MSG);
	//$name = (! isset($vars['name']) || $vars['name'] == '') ? $_no_name : $vars['name'];
	$name = (WikiParam::getVar('name') == '') ? $_no_name : WikiParam::getVar('name');
	...
	$date = (WikiParam::getVar('nodate') != '1') ? str_replace('$now', WikiConfig::getNow(), PLUGIN_PCOMMENT_FORMAT_NOW) : '';
	if ($date != '' || $name != '') {
