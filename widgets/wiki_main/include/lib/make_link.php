...
EOD;
	...
	//		$name = '<a id="notetext_' . $id . '" href="' . $script . '#notefoot_' . $id . '" class="note_super"' . $title . '>*' . $id . '</a>';				// ジャンプ先
			$name = '<a id="notetext_' . $id . '" href="' . $script . '#notefoot_' . $id . '" class="note_super"' . $title . ' data-toggle="tooltip">*' . $id . '</a>';				// ジャンプ先
		} else {
			$name = '';				// ジャンプ先
		}
...
//		return '<a href="' . $this->url . $this->anchor . '" title="' . $this->name . '" rel="nofollow">' . $this->alias . '</a>';
		return '<a href="' . $this->url . $this->anchor . '" title="' . $this->name . '" data-toggle="tooltip">' . $this->alias . '</a>';
	}
}
...
//		$dataDir = $gEnvManager->getCurrentWidgetIncludePath() . '/data';
		...
		@list($auto, $auto_a, $forceignorepages) = file($dataDir . '/autolink.dat');
		$this->auto   = $auto;
		$this->auto_a = $auto_a;
...
function make_pagelink($page, $alias = '', $anchor = '', $refer = '', $isautolink = FALSE)
{
	...
	if ($isautolink || WikiPage::isPage($page)) {
		...
		return $al_left . '<a ' . 'href="' . $script . '?' . $r_page . $anchor . '"' . $title . ' data-toggle="tooltip">' . $s_alias . '</a>' . $al_right;
	} else {
