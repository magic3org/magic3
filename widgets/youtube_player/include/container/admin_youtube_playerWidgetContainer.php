 ...
 * @version    SVN: $Id: admin_youtube_playerWidgetContainer.php 3860 2010-11-24 07:34:24Z fishbone $
 * @link       http://www.magic3.org
 */
require_once($gEnvManager->getContainerPath() . '/baseAdminWidgetContainer.php');

class admin_youtube_playerWidgetContainer extends BaseAdminWidgetContainer
{
	...
	function createDetail($request)
	{
		...
		$movieids = $request->trimValueOf('item_movieid');		// 表示項目、動画ID
		$this->theme = $request->trimValueOf('item_theme');			// 配色用テーマ
		$width = $request->trimValueOf('item_width');		// 幅
		$height = $request->trimValueOf('item_height');		// 高さ
		...
		if ($act == 'add'){// 新規追加
			...
			if ($this->getMsgCount() == 0){
				...
				$newObj->theme		= $this->theme;		// 配色用テーマ
			...
			if ($this->getMsgCount() == 0){			// エラーのないとき
				...
				if ($ret){
					// ウィジェットオブジェクト更新
					$targetObj->theme		= $this->theme;		// 配色用テーマ
					$targetObj->width		= $width;		// 幅
					$targetObj->height		= $height;		// 高さ
		...
		if (empty($this->configId)){		// 新規登録の場合
			...
			if ($replaceNew){// データ再取得時
				...
				if ($ret){
					...
					$this->theme = $targetObj->theme;		// 配色用テーマ
		...
		// jQueryUIテーマ選択メニュー作成
		$this->createThemeMenu($this->gEnv->getSystemRootPath() . self::DEFAULT_THEME_DIR);
		...
		// 画面にデータを埋め込む
		$this->tmpl->addVar("_widget", "width", $width);// 幅
		$this->tmpl->addVar("_widget", "height", $height);// 高さ
	...
	function createFieldList()
	{
		...
		for ($i = 0; $i < $fieldCount; $i++){
			...
			$row = array(
				...
				'movieid' => $this->convertToDispString($movieid),	// 動画ID
