 ...
 * @version    SVN: $Id: admin_youtube2WidgetContainer.php 2266 2009-08-28 08:25:59Z fishbone $
 * @link       http://www.magic3.org
 */
require_once($gEnvManager->getContainerPath() . '/baseAdminWidgetContainer.php');

class admin_youtube2WidgetContainer extends BaseAdminWidgetContainer
{
	...
	const DEFAULT_MOVIE_ID = 'J_W0_VrUSBU';		// デフォルトの動画
	...
	function createDetail($request)
	{
		...
		if (empty($act)){// 初期起動時
			...
			if ($this->getMsgCount() == 0){
				...
				$newObj->movieId = $movieId;		// YouTubeの動画ID
			...
			if ($this->getMsgCount() == 0){			// エラーのないとき
				...
				if ($ret){
					...
					$targetObj->movieId	= $movieId;		// YouTubeの動画ID
				}
		...
		if (empty($this->configId)){		// 新規登録の場合
			...
			if ($replaceNew){		// データ再取得時
				...
				$movieId = self::DEFAULT_MOVIE_ID;		// YouTubeの動画ID
			}
			$this->serialNo = 0;
		} else {
			if ($replaceNew){
