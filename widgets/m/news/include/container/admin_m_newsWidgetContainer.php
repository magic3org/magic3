 ...
 * @version    SVN: $Id: admin_m_newsWidgetContainer.php 2959 2010-03-22 05:01:38Z fishbone $
 * @link       http://www.magic3.org
 */
require_once($gEnvManager->getContainerPath() . '/baseWidgetContainer.php');

class admin_m_newsWidgetContainer extends BaseWidgetContainer
{
	...
	function createDetail($request)
	{
		...
		$content	= $request->valueOf('item_content');		// コンテンツ内容
		...
		if (empty($act)){// 初期起動時
			...
			if ($this->getMsgCount() == 0){
				...
				$this->gInstance->getTextConvManager()->convToEmojiTag($content, $content);
				
				// 追加オブジェクト作成
				$newObj = new stdClass;
				$newObj->name		= $name;// 表示名
				$newObj->content	= $content;		// コンテンツ内容
			...
			if ($this->getMsgCount() == 0){			// エラーのないとき
				...
				if ($ret){
					// ウィジェットオブジェクト更新
					$targetObj->content	= $content;		// コンテンツ内容
				}
		...
		if (empty($this->configId)){		// 新規登録の場合
			...
			if ($replaceNew){
				...
				if ($ret){
					...
					$content = $targetObj->content;		// コンテンツ内容
					
					// コンテンツの変換
					$this->gInstance->getTextConvManager()->convFromEmojiTag($content, $content);// Magic3内部タグから絵文字画像タグに変換
				}
			}
		...
		$this->tmpl->addVar("_widget", "content",	$content);			// コンテンツ内容
