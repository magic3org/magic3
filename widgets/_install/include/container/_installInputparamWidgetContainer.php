...
class _installInputparamWidgetContainer extends _installBaseWidgetContainer
{
    ...
    function _assign($request, &$param)
    {
        ...
        if (!$canUsePgsql && !$canUseMysql) {
            $this->setMsg(self::MSG_USER_ERR, $this->_('Can\'t use database.'));
        } // DBが使用できません
        ...
        if (empty($act)) {
            ...
            } elseif (!$canUsePgsql && $canUseMysql) {
            ...
            if ($this->gConfig->isConfigured()) {
                ...
                if (empty($sytemRootUrl)) {
                    $sytemRootUrl = 'http://' . $_SERVER['SERVER_NAME'] . '/' . M3_SYSTEM_ROOT_DIR_NAME;
                }
            ...
            if ($dbtype != M3_DB_TYPE_PGSQL && $dbtype != M3_DB_TYPE_MYSQL) {
                // DBが選択されていないとき
            ...
            if ($this->getMsgCount() == 0) {
                ...
                if ($act == 'save') {
                    ...
                    if ($db->testDbConnection($dsn, $dbuser, $password)) {
                        ...
                        $msg =
                            '<b><font color="green">' . $this->_('Succeeded in connecting database.') . '</font></b>'; // 接続正常
                        ...
                        if ($dbtype == M3_DB_TYPE_MYSQL) {
                            ...
                            if ($ret && !empty($mode)) {
                                // 取得可能な場合のみチェック
                                if (
                                    strpos($mode, 'NO_ZERO_DATE') !== false ||
                                    strpos($mode, 'STRICT_TRANS_TABLES') !== false
                                ) {
                                    $this->setMsg(
                                        self::MSG_APP_ERR,
                                        $this->_('Invalid sql_mode. Remove NO_ZERO_DATE, STRICT_TRANS_TABLES values.') .
                                            '<br />sql_mode=' .
                                            $mode
                                    ); // sql_modeが不正です。空文字列に設定して下さい。
                            ...
                            if (!$isErr && $lang == 'ja') {
                                ...
                                if ($isErr) {
                                    $this->setMsg(
                                        self::MSG_APP_ERR,
                                        $this->_('Invalid encoding for Japanese.') . '<br />' . $errMsg
                        ...
                        if ($isErr) {
                            ...
                            if ($db->testDbTable($dsn, $dbuser, $password)) {
                                $msg .=
                                    '<b><font color="green">' .
                                    $this->_('Succeeded in creating table.') .
                                    '</font></b>'; // テーブル作成正常
                            } else {
        ...
        if (defined('M3_INSTALL_PRE_FIXED_DB') && M3_INSTALL_PRE_FIXED_DB) {
            // DB接続を固定する場合
            $hostname = defined('M3_INSTALL_DB_HOST') ? M3_INSTALL_DB_HOST : '';
        ...
        } elseif ($dbtype == M3_DB_TYPE_MYSQL) {
        ...
        if ($this->dbFixed) {
            ...
            $this->tmpl->addVar('static_hostname', 'hostname', $hostname);

            $this->tmpl->setAttribute('static_dbname', 'visibility', 'visible');
            $this->tmpl->addVar('static_dbname', 'dbname', $dbname);

            $this->tmpl->setAttribute('static_dbuser', 'visibility', 'visible');
            $this->tmpl->addVar('static_dbuser', 'dbuser', $dbuser);

            if (defined('M3_INSTALL_DB_PASSWORD') && strlen(M3_INSTALL_DB_PASSWORD) > 0) {
            ...
            $this->tmpl->setAttribute('input_dbname', 'visibility', 'visible');
            $this->tmpl->addVar('input_dbname', 'dbname', $dbname);
        ...
        // 設定ファイルの内容をみて、ボタンを制御
        if (!$this->gConfig->isConfigured() && !$isConfigured) {
