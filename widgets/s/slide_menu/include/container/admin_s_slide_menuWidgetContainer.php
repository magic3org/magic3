 ...
 * @version    SVN: $Id: admin_s_slide_menuWidgetContainer.php 4947 2012-06-08 02:15:04Z fishbone $
 * @link       http://www.magic3.org
 */
require_once($gEnvManager->getContainerPath() . '/baseAdminWidgetContainer.php');
require_once($gEnvManager->getCurrentWidgetDbPath() . '/s_slide_menuDb.php');

class admin_s_slide_menuWidgetContainer extends BaseAdminWidgetContainer
{
	...
	function createDetail($request)
	{
		...
		// メニューID選択メニュー作成
		$this->db->getMenuIdList(2/*スマートフォン用メニュー*/, array($this, 'menuIdListLoop'));
	...
	{
		list($tmp, $cssHead) = explode('/', $this->gEnv->getCurrentWidgetId());		// 先頭の「s/」を削除
		return $cssHead . '_' . $this->getTempConfigId($this->paramObj);
	}
	/**
	 * 取得したデータをテンプレートに設定する
	 *
