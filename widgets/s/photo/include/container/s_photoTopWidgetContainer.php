...
require_once($gEnvManager->getCurrentWidgetDbPath() .	'/s_photo_categoryDb.php');
require_once($gEnvManager->getCurrentWidgetDbPath() . '/s_photo_commentDb.php');
require_once($gEnvManager->getCommonPath() . '/valueCheck.php');

class s_photoTopWidgetContainer extends s_photoBaseWidgetContainer
{
	...
	function _assign($request, &$param)
	{
		...
		$this->viewCount = self::$_configArray[photoCommonDef::CF_PHOTO_LIST_ITEM_COUNT];	// 表示項目数
		$this->viewOrder = self::$_configArray[photoCommonDef::CF_PHOTO_LIST_ORDER];	// 表示順
		$this->shortTitleLength = self::$_configArray[photoCommonDef::CF_PHOTO_TITLE_SHORT_LENGTH];		// 略式写真タイトルの長さ
	...
	function _addScriptLibToHead($request, &$param)
	{
		...
		if (empty(self::$_configArray[photoCommonDef::CF_PHOTO_CATEGORY_PASSWORD])){
			return '';
		} else {
			return 'md5';
		}
	...
	{
		// DBオブジェクト
		$this->categoryDb = new s_photo_categoryDb();
		...
		if (empty($paramObj)){		// 保存値がないとき
			...
			// デフォルトの検索テンプレート作成
			$searchTemplate = $this->getParsedTemplateData(photoCommonDef::DEFAULT_SEARCH_AREA_TMPL, array($this, '_makeSearcheTemplate'));// デフォルト用の検索テンプレート
		} else {
		...
		if (!empty(self::$_configArray[photoCommonDef::CF_PHOTO_CATEGORY_PASSWORD])){
			...
			// ブラウザのクライアントIDを取得
			$cid = $this->gAccess->getClientId();
			if (!empty($cid)){
				// クライアントパラメータオブジェクトを取得
				$clientObj = $this->getClientParamObj($cid, $this->gEnv->getCurrentWidgetId());
				...
				if ($act == 'addcategory'){			// 参照カテゴリー追加のとき
					...
					if ($ret && !empty($row['hc_password']) && $row['hc_password'] == $password){
						...
						$ret = $this->updateClientParamObj($cid, $this->gEnv->getCurrentWidgetId(), $clientObj);
...
//			$this->tmpl->addVar("category_area", "auth_category_list", $this->convertToDispString($this->authCategoryStr));
		...
		// ##### 検索画面作成 #####
		if (!empty(self::$_configArray[photoCommonDef::CF_PHOTO_CATEGORY_PASSWORD]) && empty($this->authCategory)){// 画像カテゴリーのパスワード制限ありで参照可能カテゴリーがないとき
			$fieldOutput = '';		// 検索画面なし
		} else {
		...
		if (!empty($fieldOutput)){
			...
			$this->tmpl->addVar("search_area", "search_form_id",	photoCommonDef::SEARCH_FORM_ID);		// 検索フォームのタグID
			/*
			$this->tmpl->addVar("_widget", "search_text_id",	photoCommonDef::SEARCH_TEXT_ID);		// 検索用テキストフィールドのタグID
			$this->tmpl->addVar("_widget", "search_button_id",	photoCommonDef::SEARCH_BUTTON_ID);		// 検索用ボタンのタグID
			$this->tmpl->addVar("_widget", "search_reset_id",	photoCommonDef::SEARCH_RESET_ID);		// 検索エリアリセットボタンのタグID
			$this->tmpl->addVar("_widget", "search_sort_id",	photoCommonDef::SEARCH_SORT_ID);		// 検索エリアソート順のタグID
			$this->tmpl->addVar("_widget", "search_form_id",	photoCommonDef::SEARCH_FORM_ID);		// 検索フォームのタグID
			...
			*/
			...
			$this->tmpl->addVar("search_script", "search_text_id",	photoCommonDef::SEARCH_TEXT_ID);		// 検索用テキストフィールドのタグID
			$this->tmpl->addVar("search_script", "search_button_id",	photoCommonDef::SEARCH_BUTTON_ID);		// 検索用ボタンのタグID
			$this->tmpl->addVar("search_script", "search_reset_id",	photoCommonDef::SEARCH_RESET_ID);		// 検索エリアリセットボタンのタグID
			$this->tmpl->addVar("search_script", "search_sort_id",	photoCommonDef::SEARCH_SORT_ID);		// 検索エリアソート順のタグID
			$this->tmpl->addVar("search_script", "search_form_id",	photoCommonDef::SEARCH_FORM_ID);		// 検索フォームのタグID
		...
		$this->tmpl->addVar("_widget", "photo_detail_url", $this->getUrl($this->gEnv->getDefaultSmartphoneUrl(), true/*リンク用*/));		// 詳細画面へのリンク
	...
	function getList($request)
	{
		...
		// 表示用パラメータ取得
		$this->thumbnailSize = intval(self::$_configArray[photoCommonDef::CF_DEFAULT_THUMBNAIL_SIZE]);		// デフォルトサムネールサイズ
		if ($this->thumbnailSize <= 0) $this->thumbnailSize = photoCommonDef::DEFAULT_THUMBNAIL_SIZE;
		$this->thumbailSizeArray = trimExplode(',', self::$_configArray[photoCommonDef::CF_THUMBNAIL_SIZE]);
		if (empty($this->thumbailSizeArray)) $this->thumbailSizeArray = array(photoCommonDef::DEFAULT_THUMBNAIL_SIZE);
	...
	{
		$commentDb = new s_photo_commentDb();
		...
		if ($ret){
			...
			// 画像サイズ
			$imageMaxSize = intval(self::$_configArray[photoCommonDef::CF_DEFAULT_IMAGE_SIZE]);
			if ($imageMaxSize <= 0) $imageMaxSize = photoCommonDef::DEFAULT_IMAGE_SIZE;
			...
			// 詳細画像
			$imagePath = photoCommonDef::getPublicImagePath($this->photoId);
			if (file_exists($imagePath)){
				$imageUrl = photoCommonDef::getPublicImageUrl($this->photoId);
			} else {
				$imageUrl = $this->notfoundThumbnailUrl;
			}
			
			// パーマリンク
			$permaUrl = $this->getUrl($this->gEnv->getDefaultSmartphoneUrl() . '?'. M3_REQUEST_PARAM_PHOTO_ID . '=' . $this->photoId);
			$permaLink = '<a href="' . $this->convertUrlToHtmlEntity($permaUrl) . '" data-ajax="false"><img src="' . $this->getUrl($this->gEnv->getRootUrl() . self::PERMALINK_ICON_FILE) . 
							'" width="' . photoCommonDef::BUTTON_ICON_SIZE . '" height="' . photoCommonDef::BUTTON_ICON_SIZE . '" title="パーマリンク" alt="パーマリンク" style="border:none;margin:0;padding:0;" /></a>';
			...
			$isRegistered = $commentDb->isExistsComment($this->photoId, $ip, $this->_langId);
			if ($isRegistered) $rateDisabled = 'disabled';
			
			// コメントを取得
			$commentDb->getCommentByPublicPhotoId(photoCommonDef::DEFAULT_COMMENT_COUNT, 1/**/, $this->photoId, $this->_langId, array($this, 'commentListLoop'));
			...
			// オンラインショップ機能
			if (!empty(self::$_configArray[photoCommonDef::CF_ONLINE_SHOP])){
				$this->tmpl->setAttribute('product_script', 'visibility', 'visible');
				$this->tmpl->setAttribute('product_info', 'visibility', 'visible');
				
				// フォト商品の表示
				if (!empty(self::$_configArray[photoCommonDef::CF_SELL_PRODUCT_PHOTO])){		// フォト関連商品販売をする場合
				...
				// ダウンロード販売の表示
				if (!empty(self::$_configArray[photoCommonDef::CF_SELL_PRODUCT_DOWNLOAD])){		// ダウンロード販売をする場合
					// ダウンロード権のチェック
			...
			$this->tmpl->addVar("photo_info_area", "author", $this->convertToDispString($author));
			$this->tmpl->addVar("photo_info_area", "place", $this->convertToDispString($row['ht_location']));
			$this->tmpl->addVar("photo_info_area", "camera", $this->convertToDispString($row['ht_camera']));
			$this->tmpl->addVar("photo_info_area", "note", $this->convertToDispString($row['ht_description']));
			$this->tmpl->addVar("photo_info_area", "date", $this->convertToDispDate($date));
			$this->tmpl->addVar("photo_info_area", "category", $this->convertToDispString($categoryStr));
			...
			$this->tmpl->addVar("photo_info_area", "permalink", $permaLink);
			...
			$this->tmpl->addVar("photo_info_area", "icon_size", photoCommonDef::BUTTON_ICON_SIZE);
			...
			$this->tmpl->addVar("photo_info_area", "rate_disabled", $rateDisabled);			// 「投票する」ボタン使用可否
			...
			if (!$this->gEnv->isSystemManageUser()){		// システム運用者以上の場合はカウントしない
				// 詳細な参照情報
				$this->gInstance->getAnalyzeManager()->updateContentViewCount(photoCommonDef::REF_TYPE_CONTENT, $row['ht_id'], $this->currentDay, $this->currentHour);
	...
	function rate($request)
	{
		...
		if (!empty($postTicket) && $postTicket == $request->getSessionValue(M3_SESSION_POST_TICKET)){		// 正常なPOST値のとき
			...
			if (0 < $rateValue && $rateValue <= 5){
				...
				$isRegistered = $commentDb->isExistsComment($this->photoId, $ip, $this->_langId);
				if (!$isRegistered){
					$ret = $commentDb->addCommentItem($this->photoId, $this->_langId, $ip, $rateValue, $comment, $rateAverage, $newSerial);
	...
	{
		// Eコマースライブラリオブジェクト取得
		$ecLibObj = $this->gInstance->getObject(photoCommonDef::EC_LIB_OBJ_ID);
	...
	function downloadImage($request)
	{
		...
		// ダウンロード処理
		$imagePath = $this->gEnv->getIncludePath() . photoCommonDef::PHOTO_DIR . $row['ht_dir'] . DIRECTORY_SEPARATOR . $row['ht_public_id'];
		if (file_exists($imagePath)){
			$ret = $this->gPage->downloadFile($imagePath, $downloadFilename);
			
			// ダウンロード数更新
			if (!$this->gEnv->isSystemManageUser()){		// システム運用者以上の場合はカウントしない
				$this->gInstance->getAnalyzeManager()->updateContentViewCount(photoCommonDef::REF_TYPE_DOWNLOAD, $row['ht_id'], $this->currentDay, $this->currentHour);
	...
	function viewListLoop($index, $fetchedRow, $param)
	{
		...
		$thumbnailPath = photoCommonDef::getThumbnailPath($fetchedRow['ht_public_id'], $thumbnailSize);
		if (file_exists($thumbnailPath)){
			$thumbnailUrl = $this->getUrl(photoCommonDef::getThumbnailUrl($fetchedRow['ht_public_id'], $thumbnailSize));
		} else {
			// 他のサイズのサムネールを探す
	...
	function prevNextListLoop($index, $fetchedRow, $urlParams)
	{
		if ($index == 0 && $fetchedRow['ht_public_id'] != $this->photoId){		// 「前へ」リンクのとき
			...
			$this->tmpl->addVar("prev_link", "icon_size", photoCommonDef::BUTTON_ICON_SIZE);
			
			// リンク作成
			$linkUrl = $this->getUrl($this->gEnv->getDefaultSmartphoneUrl() . '?'. M3_REQUEST_PARAM_PHOTO_ID . '=' . $fetchedRow['ht_public_id'] .
			...
			$this->tmpl->addVar("next_link", "icon_size", photoCommonDef::BUTTON_ICON_SIZE);
	...
	{
		$tmpl->addVar("_tmpl", "search_text_id",	photoCommonDef::SEARCH_TEXT_ID);		// 検索用テキストフィールドのタグID
		$tmpl->addVar("_tmpl", "search_button_id",	photoCommonDef::SEARCH_BUTTON_ID);		// 検索用ボタンのタグID
		$tmpl->addVar("_tmpl", "search_reset_id",	photoCommonDef::SEARCH_RESET_ID);		// 検索エリアリセットボタンのタグID
		$tmpl->addVar("_tmpl", "search_sort_id",	photoCommonDef::SEARCH_SORT_ID);		// 検索エリアソートメニューのタグID
	}
	/**
	 * ページ番号計算処理
	 *
		...
		for ($i = 0; $i < $fieldCount; $i++){
			...
			if ($itemType == 'category'){		// カテゴリーの場合
				...
				if (empty($this->menuAuthorRows)){
					$ret = self::$_mainDb->getAllUserForMenu(photoCommonDef::USER_OPTION, $this->menuAuthorRows);
				}
				$selItems = array();
	...
	function checkInputValues($category, $author, $sort)
	{
		...
		if (!in_array($this->sortKey, array('index', 'date', 'rate', 'ref')) || !in_array($this->sortDirection, array('0', '1'))){
			...
			$this->sortKey = self::$_configArray[photoCommonDef::CF_PHOTO_LIST_SORT_KEY];
			if ($this->sortKey == '') $this->sortKey = photoCommonDef::DEFAULT_PHOTO_LIST_SORT_KEY;
			$this->sortDirection = self::$_configArray[photoCommonDef::CF_PHOTO_LIST_ORDER];		
			if ($this->sortDirection == '') $this->sortDirection = photoCommonDef::DEFAULT_PHOTO_LIST_ORDER;// デフォルトの画像一覧並び順(降順)
			$retStatus = false;
		}
		return $retStatus;
	}
	...
	{
		//$linkOption = 'data-role="button" ';
		$linkOption = 'data-role="button" data-ajax="false" ';		// jQuery MobileのAjaxでの画面遷移機能をオフ
		...
		if ($pageCount > 1){	// ページが2ページ以上のときリンクを作成
			...
			for ($i = 1; $i <= $maxPageCount; $i++){
				if ($i == $pageNo){
					...
					$linkUrl = $this->getUrl($baseUrl . '&page=' . $i . $urlParams, true/*リンク用*/);
		...
		if ($pageNo > 1){		// 前ページがあるとき
			$linkUrl = $this->getUrl($baseUrl . '&page=' . ($pageNo -1) . $urlParams, true/*リンク用*/);
