 ...
 * @version    SVN: $Id: admin_s_jquery_initWidgetContainer.php 4647 2012-02-01 14:09:31Z fishbone $
 * @link       http://www.magic3.org
 */
require_once($gEnvManager->getContainerPath() . '/baseAdminWidgetContainer.php');

class admin_s_jquery_initWidgetContainer extends BaseAdminWidgetContainer
{
	...
	const DEFAULT_SCRIPT = '// init script here.';		// デフォルトのスクリプト
	...
	function createDetail($request)
	{
		...
		$script	= $request->valueOf('item_script');		// スクリプト
		...
		if (empty($this->configId)){		// 新規登録の場合
			...
			if ($replaceNew){		// データ再取得時
				...
				$script = self::DEFAULT_SCRIPT;		// スクリプト
			}
			$this->serialNo = 0;
		} else {
			if ($replaceNew){// データ再取得時
		...
		$this->tmpl->addVar("_widget", "script",	$script);					// スクリプト
