...
class admin_s_jquery_footerWidgetContainer extends BaseAdminWidgetContainer
{
	...
	const DEFAULT_CONTENT = '<div data-role="navbar"><ul><li><a href="#">One</a></li><li><a href="#">Two</a></li><li><a href="#">Three</a></li></ul></div>';		// デフォルトのフッタ内容
	...
	function createDetail($request)
	{
		...
		$content	= $request->valueOf('item_content');		// フッタ内容
...
//			$content = $this->gInstance->getTextConvManager()->deleteTag($content, 'div');
			...
			if ($this->getMsgCount() == 0){			// エラーのないとき
				...
				if ($ret){
					// ウィジェットオブジェクト更新
					$targetObj->content	= $content;		// フッタ内容
				}
		...
		if (empty($this->configId)){		// 新規登録の場合
			...
			if ($replaceNew){		// データ再取得時
				...
				$content = self::DEFAULT_CONTENT;		// フッタ内容
			}
			$this->serialNo = 0;
		} else {
			if ($replaceNew){
				...
				if ($ret){
					...
					$content = $targetObj->content;		// フッタ内容
				}
			}
		...
		$this->tmpl->addVar("_widget", "content",	$content);			// フッタ内容
