 ...
 * @version    SVN: $Id: s_bbs_2chBaseWidgetContainer.php 4878 2012-04-22 14:22:38Z fishbone $
 * @link       http://www.magic3.org
 */
require_once($gEnvManager->getContainerPath() . '/baseWidgetContainer.php');
require_once($gEnvManager->getCurrentWidgetDbPath() . '/s_bbs_2chDb.php');

class s_bbs_2chBaseWidgetContainer extends BaseWidgetContainer
{
	...
	protected $_currentPageUrl_short;	// 現在のページのURL(短縮)
	protected $_autolink;		// リンクを自動作成
	const PAGE_CONTENT_TYPE = 'bbs';		// ページコンテンツタイプ(BBS)
	...
	function __construct()
	{
		...
		if ($this->gPage->getContentType() == self::PAGE_CONTENT_TYPE){
			$this->_currentPageUrl_short = $this->gEnv->createCurrentPageUrl(false) . '?';		// ページサブIDなしの現在のページのURL
		} else {
			$this->_currentPageUrl_short = $this->_currentPageUrl . '&';
		}
	}
	/**
	 * テンプレートにデータ埋め込む
	...
	function convDispMessage($message, $threadId)
	{
		...
		if (!empty($this->_autolink)){		// 自動リンク作成のとき
			...
			// メッセージへのリンク
			$baseUrl = $this->convertUrlToHtmlEntity($this->getUrl($this->_currentPageUrl_short . M3_REQUEST_PARAM_BBS_THREAD_ID . '=' . $threadId, true));
