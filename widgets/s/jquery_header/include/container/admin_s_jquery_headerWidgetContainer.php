 ...
 * @version    SVN: $Id: admin_s_jquery_headerWidgetContainer.php 4543 2012-01-01 12:36:29Z fishbone $
 * @link       http://www.magic3.org
 */
require_once($gEnvManager->getContainerPath() . '/baseAdminWidgetContainer.php');

class admin_s_jquery_headerWidgetContainer extends BaseAdminWidgetContainer
{
	...
	const DEFAULT_CONTENT = '<h1>Header[#TITLE#]</h1><a href="#" data-transition="pop" data-icon="grid" class="ui-btn-right">About</a>';		// デフォルトのヘッダ内容
	...
	function createDetail($request)
	{
		...
		$content	= $request->valueOf('item_content');		// ヘッダ内容
		$autoBackButton = ($request->trimValueOf('item_auto_back_button') == 'on') ? 1 : 0;		// 自動的に戻るボタンを表示するかどうか
		...
		if (empty($act)){// 初期起動時
			...
			// Pタグ、DIVタグ削除
			$content = $this->gInstance->getTextConvManager()->deleteTag($content, 'p');
			$content = $this->gInstance->getTextConvManager()->deleteTag($content, 'div');
			
			// 入力チェック
			$this->checkInput($name, '名前');
			$this->checkInput($content, 'ヘッダ内容');
			...
			if ($this->getMsgCount() == 0){
				...
				$newObj->content	= $content;		// ヘッダ内容
				$newObj->autoBackButton	= $autoBackButton;		// 自動的に戻るボタンを表示するかどうか
			...
			if ($this->getMsgCount() == 0){			// エラーのないとき
				...
				if ($ret){
					// ウィジェットオブジェクト更新
					$targetObj->content	= $content;		// ヘッダ内容
					$targetObj->autoBackButton	= $autoBackButton;		// 自動的に戻るボタンを表示するかどうか
				}
		...
		if (empty($this->configId)){		// 新規登録の場合
			...
			if ($replaceNew){		// データ再取得時
				...
				$content = self::DEFAULT_CONTENT;		// ヘッダ内容
				$autoBackButton = 1;		// 自動的に戻るボタンを表示するかどうか
			}
			$this->serialNo = 0;
		} else {
			if ($replaceNew){
				...
				if ($ret){
					...
					$content = $targetObj->content;		// ヘッダ内容
					$autoBackButton = $targetObj->autoBackButton;		// 自動的に戻るボタンを表示するかどうか
				}
			}
		...
		$this->tmpl->addVar("_widget", "content",	$content);			// ヘッダ内容
		if (!empty($autoBackButton)) $this->tmpl->addVar("_widget", "auto_back_button_checked",	'checked');			// 自動的に戻るボタンを表示するかどうか
