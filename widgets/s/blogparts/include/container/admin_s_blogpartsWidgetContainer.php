...
class admin_s_blogpartsWidgetContainer extends BaseAdminWidgetContainer
{
	...
	const DEFAULT_HTML = '<a href="http://twitter.com/twitter" class="twitter-follow-button" data-show-count="false" data-lang="ja">Follow @twitter</a><script src="http://platform.twitter.com/widgets.js" type="text/javascript"></script>';		// デフォルトのブログパーツ
	...
	function createDetail($request)
	{
		...
		$html	= $request->valueOf('item_html');		// ブログパーツHTML
		...
		if ($act == 'add'){// 新規追加
			...
			if ($this->getMsgCount() == 0){			// エラーのないとき
				...
				if ($ret){
					// ウィジェットオブジェクト更新
					$targetObj->html		= $html;					// ブログパーツHTML
				}
		...
		if (empty($this->configId)){		// 新規登録の場合
			...
			if ($replaceNew){// データ再取得時
				...
				if ($ret){
					...
					$html = $targetObj->html;		// ブログパーツHTML
				}
			}
		...
		$this->tmpl->addVar("_widget", "html",	$html);					// ブログパーツHTML
