 ...
 * @version    SVN: $Id: admin_s_jquery_content_menuWidgetContainer.php 4562 2012-01-04 02:07:17Z fishbone $
 * @link       http://www.magic3.org
 */
require_once($gEnvManager->getContainerPath() . '/baseAdminWidgetContainer.php');

class admin_s_jquery_content_menuWidgetContainer extends BaseAdminWidgetContainer
{
	...
	function createDetail($request)
	{
		...
		$contentId	= $request->trimValueOf('item_content_id');		// コンテンツID
		...
		if (empty($act)){// 初期起動時
			...
			$this->checkInput($contentId, 'コンテンツID');
			...
			if ($this->getMsgCount() == 0){			// エラーのないとき
				...
				if ($ret){
					...
					$targetObj->contentId	= $contentId;		// コンテンツID
