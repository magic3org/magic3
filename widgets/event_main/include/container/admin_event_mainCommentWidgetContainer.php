...
class admin_event_mainCommentWidgetContainer extends admin_event_mainBaseWidgetContainer
{
	...
	function createDetail($request)
	{
		...
		if ($dataReload){		// データの再ロード
			...
			if ($ret){
				$this->entryId = $row['ee_id'];		// エントリーID
				$entryTitle = $row['ee_name'];				// 記事タイトル
				$name = $row['eo_name'];				// コメントタイトル
				$html = $row['eo_html'];				// HTML
				$email = $row['eo_email'];				// eメール
				$url = $row['eo_url'];				// URL
				$reg_user = $row['eo_user_name'];				// 投稿者
				$reg_dt = $this->convertToDispDateTime($row['eo_regist_dt']);				// 投稿日時
				$update_user = $row['update_user_name'];		// 更新者
				$update_dt = $this->convertToDispDateTime($row['eo_update_dt']);		// 更新日時
			}
		}
	...
	{
		// シリアル番号
		$serial = $fetchedRow['eo_serial'];
		
		// 記事ID
		$entryId = $fetchedRow['ee_id'];
		
		// 公開状態
		/*switch ($fetchedRow['eo_status']){
		...
		if (function_exists('mb_strimwidth')){
			$comment = mb_strimwidth($fetchedRow['eo_html'], 0, self::COMMENT_SIZE, '…');
		} else {
			$comment = substr($fetchedRow['eo_html'], 0, self::COMMENT_SIZE) . '...';
		}
		$row = array(
			'index' => $index,		// 項目番号
			'no' => $index + 1,													// 行番号
			'serial' => $serial,			// シリアル番号
			'entry_name' => $this->convertToDispString($fetchedRow['ee_name']),		// 記事タイトル
			'name' => $this->convertToDispString($fetchedRow['eo_name']),		// コメントタイトル
			'content' => $this->convertToDispString($comment),		// コメント内容
			'lang' => $lang,													// 対応言語
			'status' => $status,													// 公開状況
			'reg_user' => $this->convertToDispString($fetchedRow['eo_user_name']),	// 投稿者
			'reg_date' => $this->convertToDispDateTime($fetchedRow['eo_regist_dt'])	// 投稿日時
		);
