...
class admin_event_mainImageWidgetContainer extends admin_event_mainBaseWidgetContainer
{
	...
	function _assign($request, &$param)
	{
		...
		if ($act == 'update'){		// 項目更新の場合
			...
			if ($this->getMsgCount() == 0){			// エラーのないとき
				...
				// 画像を公開ディレクトリにコピー
				$publicThumbDir = $this->gInstance->getImageManager()->getSystemThumbPath(M3_VIEW_TYPE_EVENT, event_mainCommonDef::$_deviceType);
				if ($ret) $ret = cpFileToDir($privateThumbDir, $filenames, $publicThumbDir);

				// イベント記事のサムネールファイル名を更新
