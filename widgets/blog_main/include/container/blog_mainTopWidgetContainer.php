...
class blog_mainTopWidgetContainer extends blog_mainBaseWidgetContainer
{
    ...
    private $viewItemsData = []; // Joomla!ビュー用データ
    ...
    function __construct()
    {
        ...
        if (empty($this->entryViewCount)) {
            $this->entryViewCount = blog_mainCommonDef::DEFAULT_VIEW_COUNT;
        }
        ...
        if (empty($this->titleList)) {
            $this->titleList = blog_mainCommonDef::DEFAULT_TITLE_LIST;
        }
        $this->titleSearchList = self::$_configArray[blog_mainCommonDef::CF_TITLE_SEARCH_LIST]; // 検索結果タイトル
        if (empty($this->titleSearchList)) {
            $this->titleSearchList = blog_mainCommonDef::DEFAULT_TITLE_SEARCH_LIST;
        }
        $this->titleNoEntry = self::$_configArray[blog_mainCommonDef::CF_TITLE_NO_ENTRY]; // 記事なし時タイトル
        if (empty($this->titleNoEntry)) {
            $this->titleNoEntry = blog_mainCommonDef::DEFAULT_TITLE_NO_ENTRY;
        }
        $this->messageNoEntry = self::$_configArray[blog_mainCommonDef::CF_MESSAGE_NO_ENTRY]; // ブログ記事が登録されていない(404未検出)メッセージ
        if (empty($this->messageNoEntry)) {
            $this->messageNoEntry = blog_mainCommonDef::DEFAULT_MESSAGE_NO_ENTRY;
        }
        $this->messageFindNoEntry = self::$_configArray[blog_mainCommonDef::CF_MESSAGE_FIND_NO_ENTRY]; // ブログ記事が見つからない(検索結果表示用)メッセージ
        if (empty($this->messageFindNoEntry)) {
            $this->messageFindNoEntry = blog_mainCommonDef::DEFAULT_MESSAGE_FIND_NO_ENTRY;
        }
        ...
        if (empty(self::$_configArray[blog_mainCommonDef::CF_READMORE_LABEL])) {
            $this->showReadmore = false;
        } // 「もっと読む」ボタンのラベルが空の場合はボタンを非表示にする
        ...
        if (empty($this->entryListImageType)) {
            $this->entryListImageType = blog_mainCommonDef::DEFAULT_ENTRY_LIST_IMAGE_TYPE;
        }
    }
    /**
     * テンプレートファイルを設定
    ...
    function _setTemplate($request, &$param)
    {
        ...
        if (empty($this->entryId)) {
            $this->entryId = $request->trimValueOf(M3_REQUEST_PARAM_BLOG_ENTRY_ID_SHORT);
        } // 略式ブログ記事ID
        $this->blogId = $request->trimValueOf(M3_REQUEST_PARAM_BLOG_ID);
        if (empty($this->blogId)) {
            $this->blogId = $request->trimValueOf(M3_REQUEST_PARAM_BLOG_ID_SHORT);
        } // 略式ブログID
        ...
        } elseif ($act == 'search' || !empty($keyword)) {
    ...
    function _assign($request, &$param)
    {
        ...
        if ($this->isSystemManageUser && ($cmd == M3_REQUEST_CMD_PREVIEW || $cmd == M3_REQUEST_CMD_CSS)) {
            // システム運用者以上
            $this->preview = true;
        }
        ...
        switch ($this->viewMode) {
            ...
            case 10: // 記事単体表示
                ...
                $this->gInstance
                    ->getImageManager()
                    ->parseImageFormat($avatarFormat, $imageType, $imageAttr, $this->avatarSize); // 画像情報取得
        ...
        if (!empty($this->message)) {
            if ($this->_renderType == M3_RENDER_JOOMLA_NEW) {
                ...
                $this->tmpl->addVar(
                    'message',
                    'message',
                    $this->convertToDispString($this->message, true /*タグを残す*/)
                );
            }
        }

        // ##### Joomla!新型テンプレートに記事データを設定 #####
        $this->setJoomlaViewData(
            $this->viewItemsData,
            count($this->viewItemsData) /*先頭(leading部)のコンテンツ数*/,
            0 /*カラム部(intro部)のコンテンツ数*/,
            0 /*カラム部(intro部)のカラム数*/,
            $this->categoryDesc /*カテゴリーの説明*/,
            $this->readmoreLabel
        );
    ...
    function showSingle($request)
    {
        ...
        if ($this->receiveComment) {
            if ($act == 'checkcomment') {
                ...
                if ($maxCommentLength == '') {
                    $maxCommentLength = blog_mainCommonDef::DEFAULT_COMMENT_LENGTH;
                }
                if (empty($maxCommentLength)) {
                    // 空のときは長さのチェックなし
                ...
                if ($this->getMsgCount() == 0) {
                    ...
                    if (!self::$_configArray[blog_mainCommonDef::CF_COMMENT_USER_LIMITED]) {
                        // ユーザ制限なしのときは名前、メールアドレスを表示
                ...
                if (
                    ...
                    !empty($body) &&
                    !empty($postTicket) &&
                    $postTicket == $request->getSessionValue(M3_SESSION_POST_TICKET)
                ) {
                    // 正常なPOST値のとき
                    // コメントを保存
                    $ret = $this->commentDb->addCommentItem(
                        $this->entryId,
                        $this->_langId,
                        $commentTitle,
                        $body,
                    ...
                    // 記事更新日を更新
                    if ($ret) {
                        $ret = self::$_mainDb->updateEntryDt($this->entryId, $this->_langId);
                    }
                }
                if ($ret) {
            ...
            // ### コメント入力欄の表示 ###
            if (!empty($entryRow) && !empty($entryRow['be_receive_comment'])) {
                // コメントを受け付ける場合のみ表示
                if (
                    (self::$_configArray[blog_mainCommonDef::CF_COMMENT_USER_LIMITED] &&
                        $this->gEnv->isCurrentUserLogined()) ||
                    !self::$_configArray[blog_mainCommonDef::CF_COMMENT_USER_LIMITED]
                ) {
                    // ユーザ制限のときはログイン状態をチェック
        ...
        if ($this->useMultiBlog) {
            if (!empty($entryRow)) {
                // 記事レコードがあるとき
                $blogUrl = $this->getUrl(
                    $this->gEnv->getDefaultUrl() . '?' . M3_REQUEST_PARAM_BLOG_ID . '=' . $entryRow['bl_id']
                );
        ...
        if ($this->isExistsViewData) {
            // カノニカル属性を設定
            $url = $this->getUrl(
                $this->gEnv->getDefaultUrl() . '?' . M3_REQUEST_PARAM_BLOG_ENTRY_ID . '=' . $this->entryId
            );
        ...
        if (empty($category)) {
            ...
            if ($this->isSystemManageUser) {
                // システム管理ユーザの場合
                $ret = self::$_mainDb->getPrevNextEntryByKeyword(
                    $regDate,
                    $this->_langId,
                    $this->entryViewOrder,
                    $this->startDt /*期間開始*/,
            ...
            if ($this->isSystemManageUser) {
                // システム管理ユーザの場合
                $ret = self::$_mainDb->getPrevNextEntryByCategory(
                    $regDate,
                    $this->_langId,
                    $this->entryViewOrder,
                    $category,
        ...
        if ($ret) {
            ...
            if (!empty($prevRow)) {
                ...
                if (!empty($category)) {
                    $url .= '&' . M3_REQUEST_PARAM_CATEGORY_ID . '=' . $category;
                }
            ...
            if ($showPrevNextEntryLinkPos) {
                $this->setJoomlaPageNavData($prevNextEntryLinkPos, $prevUrl, $nextUrl);
            }
        }
    }
    /**
    ...
    function showTopList($request)
    {
        ...
        if (!$showTopContent) {
            ...
            if ($this->isSystemManageUser) {
                // システム管理ユーザの場合
                $totalCount = self::$_mainDb->getEntryItemsCount(
                    $this->now,
                    $this->startDt,
                    $this->endDt,
                    '' /*検索キーワード*/,
            ...
            if ($this->useMultiBlog) {
                $multiBlogParam = '&' . M3_REQUEST_PARAM_BLOG_ID . '=' . $targetBlogId;
            }
            $pageLink = $this->createPageLink(
                $this->pageNo,
                self::LINK_PAGE_COUNT,
                $this->currentPageUrl . $multiBlogParam
            );
            ...
            if ($this->isExistsViewData) {
                ...
                // ##### ページ番号遷移ナビゲーションを作成 #####
                $this->setJoomlaPaginationData(
                    $pageLinkInfo,
                    $totalCount,
                    ($this->pageNo - 1) * $this->entryViewCount /*先頭に表示する項目のオフセット番号*/,
                    $this->entryViewCount
                );
            } else {
                // ブログ記事データがないときはデータなしメッセージ追加
    ...
    function showSearchList($request)
    {
        ...
        if (empty($keyword)) {
            ...
            for ($i = 0; $i < count($parsedKeywords); $i++) {
                $this->gInstance
                    ->getAnalyzeManager()
                    ->logSearchWord($this->gEnv->getCurrentWidgetId(), $parsedKeywords[$i]);
            }
            ...
            $pageLink = $this->createPageLink(
                $this->pageNo,
                self::LINK_PAGE_COUNT,
                $this->currentPageUrl . '&act=search&keyword=' . urlencode($keyword)
            );
    ...
    function showList($request)
    {
        ...
        if (!empty($category)) {
            ...
            if ($this->isSystemManageUser) {
                ...
                $totalCount = self::$_mainDb->getEntryItemsCountByCategory(
                    $this->now,
                    $category,
                    $this->_langId,
                    $this->_userId
            ...
            $pageLink = $this->createPageLink(
                $this->pageNo,
                self::LINK_PAGE_COUNT,
                $this->currentPageUrl . '&' . M3_REQUEST_PARAM_CATEGORY_ID . '=' . $category
            );
            ...
            if ($this->isSystemManageUser) {
                // システム管理ユーザの場合
                self::$_mainDb->getEntryItemsByCategory(
                    $this->entryViewCount,
                    $this->pageNo,
                    $this->now,
                    $category,
            ...
            if (!empty($month) && !empty($day)) {
                ...
                $pageLink = $this->createPageLink(
                    $this->pageNo,
                    self::LINK_PAGE_COUNT,
                    $this->currentPageUrl . '&year=' . $year . '&month=' . $month . '&day=' . $day
                );
                ...
                $pageLink = $this->createPageLink(
                    $this->pageNo,
                    self::LINK_PAGE_COUNT,
                    $this->currentPageUrl . '&year=' . $year . '&month=' . $month
                );
    ...
    {
        $headData = ['title' => $this->headTitle, 'description' => $this->headDesc, 'keywords' => $this->headKeyword];
        return $headData;
    }
    /**
    ...
    function itemsLoop($index, $fetchedRow)
    {
        ...
        $this->gInstance
            ->getAnalyzeManager()
            ->logContentView(blog_mainCommonDef::VIEW_CONTENT_TYPE, $fetchedRow['be_serial'], $fetchedRow['be_id']);
            ...
            if ($ret) {
                ...
                for ($i = 0; $i < count($row); $i++) {
                    ...
                    if (empty($commentTitle)) {
                        $commentTitle = self::NO_COMMENT_TITLE;
                    }
                    $commentTitle =
                        '<a href="' . $permalink . '">' . $this->convertToDispString($commentTitle) . '</a>';

                    // コメント投稿ユーザ名
                    if (self::$_configArray[blog_mainCommonDef::CF_COMMENT_USER_LIMITED]) {
                        // ユーザ制限のときはログインユーザ名を取得
                    ...
                    $url = $row[$i]['bo_url'];
                    if (!empty($url)) {
                        $url = '<a href="' . $this->convertUrlToHtmlEntity($url) . '" target="_blank">URL</a>';
                    }
                    $comment =
                        '<div class="blog_comment_body">' .
                        $this->convertToPreviewText($this->convertToDispString($row[$i]['bo_html'])) .
                        '</div>'; // 改行コードをbrタグに変換
        ...
        // 記事へのリンクを生成
        $permaLinkUrl = $this->getUrl(
            $this->gEnv->getDefaultUrl() . '?' . M3_REQUEST_PARAM_BLOG_ENTRY_ID . '=' . $entryId,
            true /*リンク用*/
        ); // 記事単体
        switch ($this->showListType) {
            ...
            case 'category': // 一覧表示タイプ(カテゴリー)
                $linkUrl = $this->getUrl(
                    $this->gEnv->getDefaultUrl() .
                        '?' .
                        M3_REQUEST_PARAM_BLOG_ENTRY_ID .
                        '=' .
                        $entryId .
                        '&' .
                        M3_REQUEST_PARAM_CATEGORY_ID .
                        '=' .
                        $this->viewParam['category'],
                    true /*リンク用*/
                ); // カテゴリーID付加
                break;
            default:
        ...
        if ($viewMode == 10) {
            // 記事単体表示の場合
            $fieldInfoArray = blog_mainCommonDef::parseUserMacro(
                self::$_configArray[blog_mainCommonDef::CF_LAYOUT_ENTRY_SINGLE]
            );
        } else {
            $fieldInfoArray = blog_mainCommonDef::parseUserMacro(
                self::$_configArray[blog_mainCommonDef::CF_LAYOUT_ENTRY_LIST]
            );
        }
        $userFields = [];
        ...
        if ($isDefaltContent) {
            $thumbFilename = $defaltContentRow['be_thumb_filename'];
        }
        $thumbUrl = blog_mainCommonDef::getEyecatchImageUrl(
            $thumbFilename,
            self::$_configArray[blog_mainCommonDef::CF_ENTRY_DEFAULT_IMAGE]
        );
        ...
        if ($this->useMultiBlog && !empty($blogId)) {
            ...
            $blogLink =
                '<a href="' .
                $this->convertUrlToHtmlEntity($blogUrl) .
                '" >' .
                $this->convertToDispString($blogName) .
                '</a>';
        }
        ...
        if ($isDefaltContent) {
            $defaultSerial = $defaltContentRow['be_serial'];
        } // デフォルト言語のカテゴリーを取得
        ...
        if ($ret) {
            ...
            $categoryItems = [];
            $categoryTag .=
                '<div class="' . self::CATEGORY_BLOCK_CLASS . '"><strong>' . self::CATEGORY_BLOCK_LABEL . '</strong>'; // カテゴリー項目のラベル
            for ($i = 0; $i < count($categoryRows); $i++) {
                $categoryUrl =
                    $this->currentPageUrl . '&' . M3_REQUEST_PARAM_CATEGORY_ID . '=' . $categoryRows[$i]['bc_id']; // カテゴリーリンク先
                $categoryItems[] =
                    '<a href="' .
                    $this->convertUrlToHtmlEntity($this->getUrl($categoryUrl)) .
                    '">' .
                    $this->convertToDispString($categoryRows[$i]['bc_name']) .
                    '</a>';
            }
        ...
        // コメントへのリンク
        $commentLink = '';
        if ($this->viewMode != 10 && $this->receiveComment && $showComment) {
            // 記事単体表示でコメントを表示する場合
            $commentLink =
                '<strong><a href="' .
                $this->convertUrlToHtmlEntity($linkUrl . '#comment') .
                '" >コメント(' .
                $commentCount .
                ')</a></strong>'; // コメントへのリンク
        }
        ...
        if ($this->viewMode == 10) {
            ...
            if ($isDefaltContent) {
                $relatedContent = $defaltContentRow['be_related_content'];
            }
                ...
                if ($ret) {
                    if ($this->_renderType == M3_RENDER_JOOMLA_NEW) {
                        ...
                        $relatedContentTag .=
                            ...
                            $this->itemTagLevel .
                            '>' .
                            self::TITLE_RELATED_CONTENT_BLOCK .
                            '</h' .
                            $this->itemTagLevel .
                            '>';
                    } else {
                    ...
                    for ($i = 0; $i < count($rows); $i++) {
                        ...
                        $relatedContentTag .=
                            '<li><a href="' .
                            $this->convertUrlToHtmlEntity($this->getUrl($relatedUrl)) .
                            '">' .
                            $this->convertToDispString($rows[$i]['be_name']);
        ...
        if ($this->_renderType == M3_RENDER_JOOMLA_NEW) {
            ...
            if ($this->showEntryViewCount) {
                $viewCount = $this->gInstance
                    ->getAnalyzeManager()
                    ->getTotalContentViewCount(blog_mainCommonDef::VIEW_CONTENT_TYPE, $serial);
            }
            $contentInfo[M3_TAG_MACRO_CONTENT_VIEW_COUNT] = ''; // コンテンツ置換キー(閲覧数)
        } else {
            if ($this->showEntryAuthor) {
                // 投稿者
                $contentInfo[M3_TAG_MACRO_CONTENT_AUTHOR] = $author; // コンテンツ置換キー(著者)
            } else {
                $contentInfo[M3_TAG_MACRO_CONTENT_AUTHOR] = ''; // コンテンツ置換キー(著者)
            }
            if ($this->showEntryRegistDt) {
                // 投稿日時
            ...
            switch ($this->entryListDispType) {
                ...
                default:
                    ...
                    if (!empty($fetchedRow['be_html_ext'])) {
                        ...
                        if ($this->_renderType != M3_RENDER_JOOMLA_NEW && $this->showReadmore) {
                            $entryHtml .=
                                '<div>' .
                                self::MESSAGE_EXT_ENTRY_PRE .
                                '<a href="' .
                                $this->convertUrlToHtmlEntity($linkUrl) .
                                '" >' .
                                $this->convertToDispString($this->readmoreLabel) .
                                '</a></div>';
                        }
        ...
        if (self::$_canEditEntry) {
            $isActive = false; // 公開状態
            if ($fetchedRow['be_status'] == 2) {
                $isActive = $this->_isActive($startDt, $endDt, $this->now);
            } // 表示可能
            if (!$isActive) {
                $statusClass = ' m3content_private';
            } // 非公開記事の場合はクラス名を付加
        }
        if (!empty($entryHtml)) {
            $entryHtml = '<div class="' . self::ENTRY_BODY_BLOCK_CLASS . $statusClass . '">' . $entryHtml . '</div>';
        } // DIVで括る
        ...
        $contentParam = array_merge($userFields, [
            M3_TAG_MACRO_TITLE => '' /*タイトル*/,
            M3_TAG_MACRO_BLOG_LINK => $blogLink,
            M3_TAG_MACRO_IMAGE => $imageTag,
            M3_TAG_MACRO_BODY => $entryHtml,
            /*M3_TAG_MACRO_FILES => '', M3_TAG_MACRO_PAGES => '',*/
            'LINKS' => $relatedContentTag,
            M3_TAG_MACRO_CATEGORY => $categoryTag,
            M3_TAG_MACRO_COMMENT_LINK => $commentLink,
        ]);
        ...
        $buttonList = ''; // 記事編集ボタン
        if (self::$_canEditEntry) {
            $buttonList = $this->createButtonTag($fetchedRow['be_serial']);
        } // 編集権限があるとき
        ...
        if ($this->entryListDispType == 0) {
            // 一覧の表示タイプがコンテンツ表示(カテゴリー、関連記事付き)の場合
            ...
            if (empty($summary)) {
                // 「概要」が設定されていない場合
                // 「本文1」から画像を削除して概要を作成
                $summaryHtml = $this->convertM3ToHtml(
                    $fetchedRow['be_html'],
                    true /*改行コーをbrタグに変換*/,
                    $contentInfo
                ); // コンテンツマクロ変換
                $summaryHtml = $this->gInstance->getTextConvManager()->deleteTag($summaryHtml, ['img']); // 画像タグは削除
        ...
        // 以下は表示する項目のみ値を設定する
        if ($this->showEntryAuthor) {
            $viewItem->author = $author;
        } // 投稿者
        if ($this->showEntryRegistDt) {
            $viewItem->published = $date;
        } // 投稿日時
        if ($this->showEntryViewCount) {
            $viewItem->hits = $viewCount;
        } // 閲覧数
    ...
    function createDetailContent($viewMode, $contentParam)
    {
        ...
        if (!isset($initContentText)) {
            if ($viewMode == 10) {
                ...
                if (empty($initContentText)) {
                    $initContentText = blog_mainCommonDef::DEFAULT_LAYOUT_ENTRY_SINGLE;
                }
            } else {
                switch ($this->entryListDispType) {
                    case 0: // 記事一覧表示タイプがコンテンツ表示の場合
                    default:
                        ...
                        if (empty($initContentText)) {
                            $initContentText = blog_mainCommonDef::DEFAULT_LAYOUT_ENTRY_LIST;
                        }
                        break;
                    case 1: // 記事一覧表示タイプが概要表示の場合
    ...
    function createComment($contentParam)
    {
        static $initContentText;
        if (!isset($initContentText)) {
            $initContentText = self::$_configArray[blog_mainCommonDef::CF_LAYOUT_COMMENT_LIST];
        } // コンテンツレイアウト(コメント一覧)
        ...
        if (self::$_canEditEntry) {
            ...
            if ($this->useMultiBlog) {
                ...
                if (empty($this->blogId)) {
                    ...
                    if (isset($blogLibObj)) {
                        $bId = $blogLibObj->getBlogId();
                    }
                    if (!empty($bId)) {
                        $multiBlogParam = '&' . M3_REQUEST_PARAM_BLOG_ID . '=' . $bId;
                    }
                } else {
            ...
            if ($this->isSystemManageUser) {
                ...
                $this->headScript = $this->getParsedTemplateData(
                    'edit.tmpl.js',
                    [$this, 'makeScript'],
                    [0 /*管理者用スクリプト*/, $multiBlogParam]
                );
            } else {
                // 投稿ユーザのとき
                // 投稿管理画面へのURL作成
    ...
    function _isActive($startDt, $endDt, $now)
    {
        ...
        if ($startDt == $this->gEnv->getInitValueOfTimestamp() && $endDt == $this->gEnv->getInitValueOfTimestamp()) {
            $isActive = true; // 公開状態
        } elseif ($startDt == $this->gEnv->getInitValueOfTimestamp()) {
            if (strtotime($now) < strtotime($endDt)) {
                $isActive = true;
            } // 公開状態
        } elseif ($endDt == $this->gEnv->getInitValueOfTimestamp()) {
            if (strtotime($now) >= strtotime($startDt)) {
                $isActive = true;
            } // 公開状態
        } else {
            if (strtotime($startDt) <= strtotime($now) && strtotime($now) < strtotime($endDt)) {
                $isActive = true;
            } // 公開状態
        }
        return $isActive;
