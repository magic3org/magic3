...
class rss_event_categoryWidgetContainer extends BaseRssContainer
{
	private $db;
	...
	const DEFAULT_DESC = 'イベントカテゴリー(%s)の最新の記事が取得できます。';
...
/*		// 設定値を取得
		...
		if ($ret) $categoryName = $row['ec_name'];
	...
	function itemLoop($index, $fetchedRow)
	{
		...
		$name = $fetchedRow['ee_name'];

		// 記事へのリンク
		$linkUrl = $this->getUrl($this->defaultUrl . '?'. M3_REQUEST_PARAM_EVENT_ID . '=' . $fetchedRow['ee_id'], true);
		...
		if (!empty($name)){
			$row = array(
				...
				'date' => getW3CDate($fetchedRow['ee_start_dt'])		// イベント開始日時
			);
