...
//	const DEFAULT_SCRIPT = "jQuery(document).ready(function(\$){\n    // any code\n});\n";		// 「$」をエスケープ(2010/8/10)
	...
	function createDetail($request)
	{
		...
		$this->script	= $request->valueOf('item_script');		// Javascript
		$this->selectedPlugin = array();
		...
		if ($act == 'add'){// 新規追加
			...
			if ($this->getMsgCount() == 0){
				...
				$newObj->script	= $this->script;					// Javascript
				$newObj->selectedPlugin = $this->selectedPlugin;	// 追加するjQueryプラグイン
			...
			if ($this->getMsgCount() == 0){			// エラーのないとき
				...
				if ($ret){
					// ウィジェットオブジェクト更新
					$targetObj->script	= $this->script;					// Javascript
					$targetObj->selectedPlugin = $this->selectedPlugin;			// 追加するjQueryプラグイン
				}
		...
		if (empty($this->configId)){		// 新規登録の場合
			...
			if ($replaceNew){		// データ再取得時
				...
				$this->script = self::DEFAULT_SCRIPT;					// Javascript
				$this->selectedPlugin = array();			// 追加するjQueryプラグイン
			}
			$this->serialNo = 0;
		} else {
			if ($replaceNew){// データ再取得時
				...
				if ($ret){
					...
					$this->script = $targetObj->script;					// Javascript
					$this->selectedPlugin = $targetObj->selectedPlugin;			// 追加するjQueryプラグイン
				}
			}
		...
		$this->tmpl->addVar("_widget", "script",	$this->script);
