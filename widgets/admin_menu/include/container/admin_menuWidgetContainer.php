...
class admin_menuWidgetContainer extends BaseAdminWidgetContainer
{
    ...
    function _assign($request, &$param)
    {
        ...
        if (!$this->gEnv->isSystemManageUser()) {
            return;
        } // システム運用権限がない場合は終了(2018/8/5変更)
        ...
        if ($act == 'opensite' && !$this->gPage->isPersonalMode()) {
            // サイト公開制御。パーソナルモードの場合は変更不可。
            ...
            switch ($deviceType) {
                case 0: // PC用画面のとき
                    if ($isOpen) {
                        if ($siteInPublic) {
                            // 全サイト公開のとき
                            $this->_db->updateSystemConfig(
                                self::CF_SITE_PC_IN_PUBLIC,
                                1
                            ); // PCサイト公開
                        } else {
                            $this->_db->updateSystemConfig(
                                self::CF_SITE_IN_PUBLIC,
                                1
                            ); // サイト運用開始
                            ...
                            $this->_db->updateSystemConfig(
                                self::CF_SITE_SMARTPHONE_IN_PUBLIC,
                                0
                            ); // スマートフォンサイト公開
                        }
                    } else {
        ...
        if (!empty($menuStatus)) {
            ...
            } elseif ($menuStatus == 'true') {
                $paramObj->showMenu = 1; // メニューを表示するかどうか
            }
            $ret = $this->updateWidgetParamObj($paramObj);
        }

        $this->cssFilePath = $this->getUrl(
            $this->gEnv->getCurrentWidgetCssUrl() . self::DEFAULT_CSS_FILE
        ); // CSSファイル
        ...
        if ($menu == 'off') {
            ...
            // 別ウィンドウで表示のときは閉じるボタン表示
            if (
                $openBy != 'tabs' &&
                $openBy != 'iframe' &&
                $openBy != 'dialog'
            ) {
                // タブ、インナーフレーム、ダイアログ表示以外
                $this->useCloseButton = true; // 「閉じる」を使用するかどうか
                $this->tmpl->setAttribute(
                    'closebutton',
                    'visibility',
                    'visible'
                );
                ...
                if (!empty($server)) {
                    ...
                    if ($ret) {
                        ...
                        $serverName =
                            $this->_('Server Name:') . ' ' . $row['ts_name']; // サーバ名
                        $this->tmpl->addVar(
                            'closebutton',
                            'server_name',
                            $this->convertToDispString($serverName)
                        );
                    }
                }
            }
            ...
            if ($this->gPage->isPersonalMode()) {
                ...
                // メインメニュー表示の場合
                // システムの表示モードを取得
                $isSiteOperationModeOn = $this->gSystem->getSystemConfig(
                    self::CF_SITE_OPERATION_MODE
                ); // サイト運用モード
                ...
                $navId =
                    self::DEFAULT_NAV_ID .
                    '.' .
                    $this->gEnv->getCurrentLanguage();
                if (!$this->db->getNavItems($navId, 0, $rows)) {
                    // 現在の言語で取得できないときはデフォルト言語で取得
                    $navId =
                        self::DEFAULT_NAV_ID .
                        '.' .
                        $this->gEnv->getDefaultLanguage();
                    if (!$this->db->getNavItems($navId, 0, $rows)) {
                        // デフォルト言語で取得できないときは拡張子なしで取得
                ...
                for ($i = 0; $i < $topMenuCount; $i++) {
                    // 非表示オプション取得
                    $hideOptions = [];
                    if (!empty($rows[$i]['ni_hide_option'])) {
                        $hideOptions = explode(
                            ',',
                            $rows[$i]['ni_hide_option']
                        );
                    }

                    // サイト運用モードがオンの場合は非表示項目を非表示にする
                    if (
                        $isSiteOperationModeOn &&
                        in_array('site_operation', $hideOptions)
                    ) {
                        continue;
                    }

                    // ### 読み飛ばさない行が一行でもある場合は改行を出力する ###
                    if ($rows[$i]['ni_view_control'] == 1) {
                        // 改行のとき
                        if (!$escapeColumnEnd) {
                            $columnCount++;
                        }
                ...
                if (
                    $topMenuCount > 0 &&
                    $rows[$topMenuCount - 1]['ni_view_control'] == 0 &&
                    !$escapeColumnEnd
                ) {
                    $columnCount++;
                }
                ...
                $menuInner =
                    str_repeat(M3_INDENT_SPACE, self::MAINMENU_INDENT_LEBEL) .
                    '<li class="' .
                    self::MAINMENU_COL_STYLE .
                    $columnWidth .
                    '"><ul>' .
                    M3_NL;
                ...
                for ($i = 0; $i < $topMenuCount; $i++) {
                    ...
                    if ($rows[$i]['ni_view_control'] == 1) {
                        ...
                        // メニュー大項目
                        $menuInner .= str_repeat(
                            M3_INDENT_SPACE,
                            self::MAINMENU_INDENT_LEBEL + 1
                        );
                        $menuInner .=
                            ...
                            for ($l = 0; $l < count($subRows); $l++) {
                                ...
                                if (!empty($title)) {
                                    $helpText = $this->gInstance
                                        ...
                                        ->createHelpText(
                                            $title,
                                            $subRows[$l]['ni_help_body']
                                        );
                                }

                                $menuInner .= str_repeat(
                                    M3_INDENT_SPACE,
                                    self::MAINMENU_INDENT_LEBEL + 2
                                );
                                ...
                                switch ($itemType) {
                                    ...
                                    default:
                                        $menuInner .= '<li><a href="';
                                        $menuInner .= $this->getUrl(
                                            $this->gEnv->getDefaultAdminUrl() .
                                                '?task=' .
                                                $subRows[$l]['ni_task_id']
                                        ); // 起動タスクパラメータを設定
                                        if (!empty($subRows[$l]['ni_param'])) {
                                            // パラメータが存在するときはパラメータを追加
                                            $menuInner .=
                                                '&' .
                                                M3_REQUEST_PARAM_OPERATION_TODO .
                                                '=' .
                                                urlencode(
                                                    $subRows[$l]['ni_param']
                                                );
            ...
            $this->tmpl->addVar(
                'menu',
                'top_url',
                $this->getUrl($this->gEnv->getDefaultAdminUrl())
            ); // トップメニュー画面URL

            // ### サイト名表示 ###
            $siteName = $this->gEnv->getSiteName();
            if ($this->_isSmallDeviceOptimize) {
                // 小画面デバイス最適化の場合
                $siteName = makeTruncStr(
                    $siteName,
                    self::MAX_SITENAME_LENGTH_S
                );
            } else {
            ...
            // 小画面最適化アイコン
            if ($this->_isSmallDeviceOptimize) {
                // 管理画面の小画面デバイス最適化を行う場合
                $iconUrl =
                    $this->gEnv->getRootUrl() . self::SMALL_DEVICE_ICON_FILE;
                $iconTitle = $this->_('Small Screen Device'); // 小画面デバイス
                $imageSize = self::SITE_ICON_SIZE;
                $iconTag =
                    '<img class="smalldevice" src="' .
                    $this->getUrl($iconUrl) .
                    '" width="' .
                    $imageSize .
                    '" height="' .
            ...
            $iconUrl = '';
            $this->systemType = $this->gSystem->getSystemConfig(
                self::CF_SYSTEM_TYPE
            ); // システム運用タイプ
            $testMode = $this->gSystem->getSystemConfig(self::CF_TEST_MODE); // テストモードかどうか
            $developMode = $this->gSystem->getSystemConfig(
                self::CF_PERMIT_DETAIL_CONFIG
            ); // 開発モード
            $preTitle = '';
            if ($testMode) {
                // テストモードかどうか
                $iconUrl =
                    $this->gEnv->getRootUrl() . self::SITE_TEST_ICON_FILE; // サイトテスト中アイコン
                $preTitle = '[テスト中]'; // 開発モード
            } elseif ($developMode) {
                // 開発モードの場合
                ...
                if ($this->systemType == self::SYSTEM_TYPE_SERVER_ADMIN) {
                    // サーバ管理の場合
                    $iconUrl =
                        $this->gEnv->getRootUrl() .
                        self::TOP_SERVER_ADMIN_ICON_FILE;
                } else {
                    if ($isSiteOperationModeOn) {
                        // サイト運用モードのとき
                        $iconUrl =
                            $this->gEnv->getRootUrl() .
                            self::TOP_MENU_ICON_FILE;
                    } else {
            ...
            if (empty($iconUrl)) {
                $iconTag =
                    '<i class="dashboard fas fa-tachometer-alt text-warning"></i>'; // ダッシュボードアイコン
            } else {
                $iconTag =
                    '<img class="home" src="' .
                    $this->getUrl($iconUrl) .
            ...
            if ($this->gPage->isPersonalMode()) {
                ...
                $this->tmpl->addVar(
                    'system_version',
                    'system',
                    'Magic3 v' . M3_SYSTEM_VERSION
                );
                $this->tmpl->addVar(
                    'system_version',
                    'official_url',
                    M3_SYSTEM_OFFICIAL_SITE
                );
            }
            ...
            if (empty($userName)) {
                $userName = self::UNTITLED_USER_NAME;
            }
            $this->tmpl->addVar(
                'menu',
                'user',
                $this->convertToDispString($userName)
            );

            // アバター
            $avatarFormat = $this->gInstance
                ->getImageManager()
                ->getDefaultAvatarFormat(); // 画像フォーマット取得
            // アバター画像取得
            $this->gInstance
                ->getImageManager()
                ->parseImageFormat(
            ...
            // ユーザメニュー
            $iconTag =
                '<i class="fas fa-user fa-2x" style="color: black;"></i>'; // デフォルトカラーは黒。黒以外の場合はtext-danger等クラスを使用する。
            $this->tmpl->addVar('menu', 'user_info_img', $iconTag);
            $loginStatusUrl = '?task=userlist_detail&userid=' . $userId; // ユーザ情報画面URL
            $this->tmpl->addVar(
                'menu',
                'user_info_url',
                $this->convertUrlToHtmlEntity($loginStatusUrl)
            );
            //$iconTitle = 'ログアウト';
        ...
        if (!empty($breadcrumbDef)) {
            ...
            $breadcrumbHtml = $this->gDesign->createAdminBreadcrumb(
                $breadcrumbDef,
                $helpDef
            );
        ...
        if (!empty($subNavbarDef) || !empty($breadcrumbDef)) {
            ...
            $this->tmpl->addVar(
                'fixtoppos',
                'second_top',
                $this->convertToDispString($topPos - self::SUB_MENUBAR_HEIGHT)
            );
            $this->tmpl->addVar(
                'fixtoppos',
                'content_top',
                $this->convertToDispString($topPos)
            ); // コンテンツのトップ位置
        }
    ...
    function _addCssFileToHead($request, &$param)
    {
        ...
        } elseif ($this->useCloseButton) {
            // 「閉じる」を使用するかどうか
            $this->cssFilePath = $this->getUrl(
                $this->gEnv->getCurrentWidgetCssUrl() . self::WIDGET_CSS_FILE
            ); // ウィジェット単体表示用CSSファイル
            return $this->cssFilePath;
        } else {
            return '';
    ...
    function createSiteMenuTag()
    {
        ...
        if ($isActiveSite) {
            ...
            if ($isOpen && $sitePcInPublic) {
                ...
                $iconUrl =
                    $this->gEnv->getRootUrl() . self::PC_CLOSED_ICON_FILE; // サイト非公開
            }
            $iconTag =
                str_repeat(M3_INDENT_SPACE, self::SITEMENU_INDENT_LEBEL) .
                '<li class="dropdown" >' .
                M3_NL;
            $iconTag .=
                str_repeat(M3_INDENT_SPACE, self::SITEMENU_INDENT_LEBEL + 1) .
                '<a href="#" class="dropdown-toggle device_icon" data-toggle="dropdown" data-placement="left" data-container="body" title="' .
                $iconTitle .
                '" rel="m3help">';
            $iconTag .=
                '<img src="' .
        ...
        if ($isActiveSite) {
            ...
            if ($isOpen && $siteSmartphoneInPublic) {
                $iconUrl =
                    $this->gEnv->getRootUrl() . self::SMARTPHONE_ICON_FILE;
                $isVisibleSite = true; // 公開中かどうか
            } else {
                $iconUrl =
                    $this->gEnv->getRootUrl() .
                    self::SMARTPHONE_CLOSED_ICON_FILE; // サイト非公開
            }
            $iconTag =
                str_repeat(M3_INDENT_SPACE, self::SITEMENU_INDENT_LEBEL) .
                '<li class="dropdown" >' .
    ...
    function getContentMenu()
    {
        $menuItems = [[], [], []];
        $pageIdArray = [
            $this->gEnv->getDefaultPageId(),
            $this->gEnv->getDefaultMobilePageId(),
            $this->gEnv->getDefaultSmartphonePageId(),
        ];
        $contentType = $this->gPage->getMainContentTypes();
        $ret = $this->db->getEditWidgetOnPage(
            $pageIdArray,
            $contentType,
            $rows
        );
    ...
    function getSubContentMenu()
    {
        ...
        $ret = $this->db->getEditSubWidgetOnPage(
            $pageIdArray,
            $contentType,
            $rows
        );
    ...
    function createContentMenu($deviceType, $isVisibleSite)
    {
        ...
        if ($this->_isSmallDeviceOptimize) {
            ...
            //$iconTag = '<img src="' . $this->getUrl($iconUrl) . '" width="' . self::ICON_SIZE . '" height="' . self::ICON_SIZE . '" border="0" alt="' . $iconTitle . '" />';
            $iconTag =
                '<i class="fas fa-image fa-2x" style="color: black;"></i>';
        }
        ...
        switch ($deviceType) {
            ...
            default:
                $menuTag .=
                    '<li><a href="#" onclick="m3ShowPreviewWindow(0, \'' .
            ...
            case 2: // スマートフォン用画面のとき
                $menuTag .=
                    '<li><a href="#" onclick="m3ShowPreviewWindow(2, \'' .
                    $this->gEnv->getDefaultSmartphoneUrl() .
                    '\');return false;">' .
                    $iconTag .
                    $title .
                    '</a></li>' .
        ...
        if (!empty($menu)) {
            ...
            if ($this->_isSmallDeviceOptimize) {
                ...
                //$iconTag = '<img src="' . $this->getUrl($iconUrl) . '" width="' . self::ICON_SIZE . '" height="' . self::ICON_SIZE . '" border="0" alt="' . $iconTitle . '" />';
                $iconTag =
                    '<i class="fas fa-cog fa-2x" style="color: black;"></i>';
            }
            for ($i = 0; $i < count($menu); $i++) {
                $widgetId = $menu[$i]['wd_id'];
                $title = $this->getCurrentLangString(
                    $menu[$i]['wd_content_name']
                ); // ウィジェットのコンテンツ名を取得
                ...
                if (empty($title)) {
                    $title = $menu[$i]['wd_name'];
                } // コンテンツ名が取得できないときはウィジェット名を設定
                if (empty($title)) {
                    continue;
                }
                ...
                $menuTag .=
                    '<li ><a href="#" onclick="m3ShowConfigWindow(\'' .
                    $widgetId .
                    '\', 0, 0);return false;">' .
                    $iconTag .
        ...
        if (!empty($subMenu)) {
            ...
            for ($i = 0; $i < count($subMenu); $i++) {
                ...
                if ($subMenu[$i]['wd_content_widget_id']) {
                    $widgetId = $subMenu[$i]['wd_content_widget_id'];
                }
                $title = '';
                //$title = $this->getCurrentLangString($subMenu[$i]['wd_content_name']);		// ウィジェットのコンテンツ名を取得
                if (isset($subMenu[$i]['wd_content_name'])) {
                    $title = $this->getCurrentLangString(
                        $subMenu[$i]['wd_content_name']
                    );
                } // ウィジェットのコンテンツ名を取得
                ...
                if (empty($title)) {
                    ...
                    if (isset($mainContentTypeArray[$contentType])) {
                        $title = $mainContentTypeArray[$contentType];
                    }
                    ...
                    if (empty($title)) {
                        $title = $mainFeatureTypeArray[$contentType];
                    }
                }
                if (empty($title)) {
                    $title = $subMenu[$i]['wd_name'];
                } // サブコンテンツ名が取得できないときはウィジェット名を設定
                if (empty($title)) {
                    continue;
                }
    ...
    function createAccessPointControlMenu($deviceType, $isVisibleSite)
    {
        ...
        if (
            $this->gEnv->isSystemManager() &&
            !in_array(
                self::TASK_SITEOPEN,
                $this->gSystem->getSystemManagerEnableTask()
            )
        ) {
            return '';
        }
        ...
        if ($isVisibleSite) {
            ...
            $iconTag =
                '<i class="fas fa-caret-square-right fa-2x text-success"></i>';
        }
        $menuTag .= str_repeat(
            M3_INDENT_SPACE,
            self::SITEMENU_INDENT_LEBEL + 2
        ...
        $menuTag .=
            '<li><a href="#" onclick="siteOpen(' .
            $deviceType .
            ',' .
            intval(!$isVisibleSite) .
            ');return false;">';
    ...
    function createMenuDefMenu($deviceType, $isVisibleSite)
    {
        ...
        if ($ret) {
            ...
            for ($i = 0; $i < count($destMenu); $i++) {
                ...
                $menuTag .=
                    '<i class="fas fa-stream fa-2x" style="color: black;"></i>' .
                    $this->convertToDispString($title) .
                    '</a></li>' .
                    M3_NL;
            }
    ...
    function arrangeSubMenu($menu)
    {
        ...
        for ($i = 0; $i < count($menu); $i++) {
            $menuRow = $menu[$i];
            $widgetId = $menu[$i]['wd_id'];
            if (!empty($menu[$i]['wd_content_widget_id'])) {
                $widgetId = $menu[$i]['wd_content_widget_id'];
            }
    ...
    {
        // タイトル作成
        $titleTag = $this->gDesign->createSubMenubarTitleTag(
            $navbarDef,
            2 /*システム画面(共通設定画面等)*/
        );
