 ...
 * @version    SVN: $Id: admin_reserve_mainWidgetContainer.php 2939 2010-03-17 14:20:34Z fishbone $
 * @link       http://www.magic3.org
 */
require_once($gEnvManager->getCurrentWidgetContainerPath() . '/admin_reserve_mainBaseWidgetContainer.php');

class admin_reserve_mainWidgetContainer extends admin_reserve_mainBaseWidgetContainer
{
	/**
	 * コンストラクタ
	 */
	...
	function _dispatch($request, &$param)
	{
		...
		switch ($task){
			...
			case 'resource_detail':		// リソース設定(詳細)
				$task = 'resource';
				$goWidget = true;		// サブウィジェットを実行するかどうか
				break;	
			default:
		...
		if ($goWidget){		// サブウィジェットを実行するかどうか
			$gLaunchManager->goSubWidget($task, true);		// 管理者機能で呼び出し
			return false;
		} else {
